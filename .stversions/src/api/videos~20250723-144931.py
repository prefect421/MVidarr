...
from src.utils.security import (
    require_rate_limit, validate_request_data, InputValidator
)
...
def resolve_video_url(video, session):
    """
    Helper function to resolve video URL using multiple strategies
    ...
    # If we have a YouTube ID, construct the URL directly
    if video.youtube_id:
        youtube_url = f"https://www.youtube.com/watch?v={video.youtube_id}"
        video.url = youtube_url
        session.commit()
        logger.info(f"Constructed YouTube URL from ID for '{video.title}': {youtube_url}")
        return youtube_url
    ...
    try:
        ...
        search_queries = [
            f"{artist_name} {video_title}",
            f"{artist_name} {video_title} official",
            f"{artist_name} {video_title} music video",
            f'"{artist_name}" "{video_title}"'  # Exact phrase search
        ]
        
        for i, search_query in enumerate(search_queries):
            logger.info(f"Searching for video URL (attempt {i+1}): {search_query}")
            
            # Use yt-dlp to search YouTube with multiple options to avoid age restrictions
            cmd = [
                '/usr/local/bin/yt-dlp',
                '--dump-json',
                '--no-download',
                '--playlist-items', '1',
                '--age-limit', '0',  # Try to avoid age-restricted content
            ...
            try:
                ...
                if result.returncode == 0 and result.stdout:
                    ...
                    if resolved_url:
                        # Check if this is age-restricted by examining the error
                        if 'age' not in result.stderr.lower() and 'sign in' not in result.stderr.lower():
                            # Update video with resolved URL
                            video.url = resolved_url
                            if 'id' in video_info:
                                video.youtube_id = video_info['id']
                            session.commit()
                            logger.info(f"Resolved video URL for '{video.title}' on attempt {i+1}: {resolved_url}")
                            return resolved_url
                        else:
                            logger.warning(f"Found URL but it appears age-restricted for '{video.title}': {resolved_url}")
                    else:
                        logger.warning(f"No URL found in video info for '{video.title}' on attempt {i+1}")
                else:
                    stderr_msg = result.stderr.lower()
                    if 'age' in stderr_msg or 'sign in' in stderr_msg:
                        logger.warning(f"Age-restricted content found for '{search_query}', trying next search strategy")
                        continue
                    else:
                ...
                logger.error(f"Invalid JSON response from yt-dlp for '{search_query}'")
                continue
            
        # If all searches failed, log the final attempt
        logger.error(f"All URL resolution attempts failed for '{video.title}' by {artist_name}")
...
def find_relocated_video(video, session):
    ...
    try:
        ...
        if os.path.exists(artist_folder):
            ...
            video_title_clean = video_title.lower().replace(' ', '').replace('_', '').replace('-', '')
            ...
            for filename in os.listdir(artist_folder):
                if filename.lower().endswith(('.mp4', '.mkv', '.avi', '.webm', '.mov')):
                    ...
                    filename_clean = filename.lower().replace(' ', '').replace('_', '').replace('-', '')
                    
                    # Try different matching strategies
                    if (video_title_clean in filename_clean or 
                        filename_clean.startswith(video_title_clean[:10]) or
                        any(word in filename_clean for word in video_title_clean.split() if len(word) > 3)):
...
def _trigger_video_download(video_id):
    ...
    try:
        with get_db() as session:
            ...
            if not video_url:
                return {'success': False, 'error': 'Video has no URL to download and could not resolve one'}
            ...
            status_value = video.status.value if hasattr(video.status, 'value') else video.status
            if status_value == 'DOWNLOADED' and video.local_path and os.path.exists(video.local_path):
            ...
            if isinstance(download_subtitles, str):
                download_subtitles = download_subtitles.lower() == 'true'
            ...
            if result and result.get('success'):
                ...
                error_msg = result.get('error', 'Failed to queue download') if result else 'yt-dlp service unavailable'
...
def get_videos():
    ...
    try:
        ...
        with get_db() as session:
            query = session.query(Video).join(Artist, Video.artist_id == Artist.id, isouter=True)
...
@require_rate_limit(max_requests=200, window_seconds=3600)  # Limit searches to 200 per hour
def search_videos():
    """Search videos with multiple filters"""
    try:
        # Get and sanitize search parameters
        query = InputValidator.sanitize_string(request.args.get('q', '').strip(), max_length=200)
        artist_name = InputValidator.sanitize_string(request.args.get('artist', '').strip(), max_length=100)
        status = InputValidator.sanitize_string(request.args.get('status', '').strip(), max_length=50)
        year = request.args.get('year', '').strip()
        genre = InputValidator.sanitize_string(request.args.get('genre', '').strip(), max_length=100)
        quality = InputValidator.sanitize_string(request.args.get('quality', '').strip(), max_length=50)
        has_thumbnail = request.args.get('has_thumbnail', '').strip()
        source = InputValidator.sanitize_string(request.args.get('source', '').strip(), max_length=50)
        
        # Validate numeric parameters
        duration_min = InputValidator.validate_integer(request.args.get('duration_min'), min_val=0, max_val=86400)
        duration_max = InputValidator.validate_integer(request.args.get('duration_max'), min_val=0, max_val=86400)
        
        # Validate year parameter
        if year:
            year_int = InputValidator.validate_integer(year, min_val=1900, max_val=2100)
            if year_int is None:
...
def download_video(video_id):
    ...
    try:
        with get_db() as session:
            ...
            if not video_url:
                return jsonify({'error': 'Video has no URL to download and could not resolve one'}), 400
            ...
            if status_value == 'DOWNLOADED' and video.local_path and os.path.exists(video.local_path):
                return jsonify({
                    'success': False,
                    'error': 'Video is already downloaded'
                }), 400
...
def identify_artist(video_id):
    ...
    try:
        ...
        candidates = artist_identification_service.identify_artist_from_title(video_title)
...
def search_artists():
    ...
    try:
        ...
        with get_db() as session:
            ...
            ).order_by(Artist.name).limit(10).all()
...
def delete_video(video_id):
    ...
    try:
        with get_db() as session:
            ...
            logger.info(f"Deleted video: {video_info['title']} by {video_info['artist_name']}")
...
@require_rate_limit(max_requests=20, window_seconds=3600)  # Limit bulk deletes to 20 per hour
@validate_request_data(required_fields=['video_ids'], max_payload_size=1024*10)  # 10KB max
def bulk_delete_videos():
    ...
    try:
        ...
        # Validate video IDs - must be positive integers
        sanitized_ids = []
        for video_id in video_ids:
            validated_id = InputValidator.validate_integer(video_id, min_val=1, max_val=999999999)
            if validated_id is None:
                return jsonify({'error': f'Invalid video ID: {video_id}'}), 400
            sanitized_ids.append(validated_id)
        
        # Limit bulk operations to reasonable size
        if len(sanitized_ids) > 100:
            return jsonify({'error': 'Bulk delete limited to 100 videos at once'}), 400
        ...
        with get_db() as session:
            for video_id in sanitized_ids:
                try:
                    ...
                    logger.info(f"Bulk deleted video: {video_info['title']} by {video_info['artist_name']}")
                    ...
                    logger.error(f"Failed to delete video {video_id} in bulk operation: {e}")
    ...
    try:
        with get_db() as session:
            ...
            if video.thumbnail_path:
                ...
                if os.path.isabs(video.thumbnail_path):
                    thumbnail_path = video.thumbnail_path
                else:
                    # Resolve relative path from project root
                    thumbnail_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), video.thumbnail_path)
                
                if os.path.exists(thumbnail_path):
                    return send_file(thumbnail_path, as_attachment=False)
                else:
                    logger.warning(f"Video thumbnail file not found: {thumbnail_path}")
            ...
            if video.thumbnail_url and not video.thumbnail_path:
                try:
                    ...
                    thumbnail_path = thumbnail_service.download_video_thumbnail(artist_name, video.title, video.thumbnail_url)
                    
                    if thumbnail_path:
                        # Convert to absolute path if it's relative
                        if os.path.isabs(thumbnail_path):
                            abs_thumbnail_path = thumbnail_path
                        else:
                            abs_thumbnail_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), thumbnail_path)
                        
                        if os.path.exists(abs_thumbnail_path):
                            # Update the database with the new thumbnail path
                            video.thumbnail_path = thumbnail_path
                            session.commit()
                            logger.info(f"Downloaded and cached thumbnail for video {video_id}")
                            return send_file(abs_thumbnail_path, as_attachment=False)
            ...
            if video.thumbnail_url:
                try:
                    import requests
                    response = requests.get(video.thumbnail_url, timeout=10, stream=True)
...
def refresh_thumbnails():
    ...
    try:
        ...
        with get_db() as session:
            ...
            videos = session.query(Video).join(Artist).filter(
            ...
            for video in videos:
                try:
                    ...
                    if thumbnail_path:
                        video.thumbnail_path = thumbnail_path
                        downloaded_count += 1
                        logger.info(f"Downloaded thumbnail for video: {video.title} by {artist_name}")
                    else:
                        failed_count += 1
                        logger.warning(f"Failed to download thumbnail for video: {video.title}")
                        
                except Exception as e:
                    failed_count += 1
                    logger.error(f"Failed to download thumbnail for video {video.title}: {e}")
            
            # Also count IMVDb URLs that we're skipping
            imvdb_videos = session.query(Video).filter(
                Video.thumbnail_url.isnot(None),
                Video.thumbnail_path.is_(None),
    ...
    try:
        ...
        with get_db() as session:
            ...
            if action == 'remove':
                ...
                return jsonify({'error': 'Invalid action or missing thumbnail_url'}), 400
    ...
    try:
        ...
        with get_db() as session:
            ...
            if not video.thumbnail_path or not os.path.exists(video.thumbnail_path):
                return jsonify({'error': 'No local thumbnail file available for cropping'}), 400
        ...
        with get_db() as session:
            ...
            if 'youtube' in sources and video.url and 'youtube.com' in video.url:
                try:
                    ...
                    logger.warning(f"Failed to get YouTube thumbnails for video {video_id}: {e}")
                ...
                try:
                    ...
                    if video_details:
                        ...
                        if thumbnail_url:
                            ...
                            logger.info(f"Found IMVDb thumbnail for video {video_id}: {thumbnail_url}")
                        else:
                            logger.debug(f"No thumbnail URL found in IMVDb data for video {video_id}")
                    else:
                        logger.debug(f"No IMVDb video details found for ID {video.imvdb_id}")
                except Exception as e:
                    logger.warning(f"Failed to get IMVDb thumbnails for video {video_id}: {e}")
    ...
    try:
        with get_db() as session:
            ...
            info = {
                ...
                'thumbnail_uploaded_at': video.thumbnail_uploaded_at.isoformat() if video.thumbnail_uploaded_at else None,
...
def get_video_thumbnail_size(video_id, size):
    ...
    try:
        if size not in ['small', 'medium', 'large', 'original']:
            return jsonify({'error': 'Invalid size. Use: small, medium, large, or original'}), 400
        ...
        with get_db() as session:
            ...
            size_path = thumbnail_service.get_thumbnail_size_path(video.thumbnail_path, size)
        ...
        with get_db() as session:
            ...
            if video.local_path:
                ...
                if os.path.exists(full_path):
                    ...
                    if full_path.lower().endswith('.mkv') or full_path.lower().endswith('.avi'):
                        # Use FFmpeg streaming for MKV/AVI files
                        from src.services.ffmpeg_streaming_service import ffmpeg_streaming_service
                    ...
                    logger.info(f"Serving video {video.title} from {full_path} with MIME type: {mime_type}")
                    ...
                    if relocated_path:
                        ...
                        logger.error(f"Video file not found and could not be relocated: {full_path}")
    ...
    try:
        with get_db() as session:
            ...
            if not video.local_path:
                return jsonify({'error': 'No local file available for transcoding'}), 400
            ...
            def transcode_worker():
                try:
                    ...
                    cmd = [
                        ...
                        '-crf', '23',               # Quality (18-28, 23 is good balance)
                    ...
                    if result.returncode == 0:
                        ...
                        with get_db() as update_session:
                            update_video = update_session.query(Video).filter(Video.id == video_id).first()
                            if update_video:
                                ...
                                logger.info(f"Updated video {video_id} with transcoded path: {relative_output}")
                    else:
                        logger.error(f"Transcoding failed for video {video_id}: {result.stderr}")
    ...
    try:
        with get_db() as session:
            ...
            if os.path.exists(transcoded_path):
                return jsonify({'status': 'completed', 'transcoded_file': transcoded_path}), 200
            else:
                return jsonify({'status': 'not_started', 'format': Path(video.local_path).suffix}), 200
...
def refresh_video_metadata(video_id):
    ...
    try:
        ...
        if current_imvdb_id:
            logger.info(f"Attempting to refresh using existing IMVDb ID: {current_imvdb_id}")
        ...
        with get_db() as session:
            ...
            if not video:
                return jsonify({'error': 'Video was deleted during metadata refresh'}), 404
        ...
        with get_db() as session:
            ...
            for video in problematic_videos:
                try:
                    ...
                    common_artists = ['Unknown', 'Various Artists', 'Compilation', 'Soundtrack']
                    if current_artist_name in common_artists:
                        logger.debug(f"Skipping video {video.id} - artist name '{current_artist_name}' looks like actual artist")
                        continue
                    
                    # The fix: swap artist name and title
                    new_title = current_artist_name  # Move artist name to title
                    new_artist_name = 'Unknown'  # Reset artist to Unknown (will be fixed later)
                    ...
                    if not dry_run:
                        ...
                        if new_artist_name != 'Unknown':
                            artist = session.query(Artist).filter(Artist.name == new_artist_name).first()
                            if not artist:
                        ...
                        # Log the change
                        logger.info(f"Fixed video {video.id}: '{current_artist_name}' -> '{new_artist_name}' | '{current_title}' -> '{new_title}'")
            ...
            logger.info(f"Title/Artist swap {'dry run' if dry_run else 'fix'} completed: {processed} processed, {fixed} fixed, {errors} errors")
            ...
            return jsonify({
                ...
                'fixed': fixed,
                'errors': errors,
                'changes': changes[:20] if dry_run else [],  # Show first 20 changes for dry run
...
def refresh_all_metadata():
    ...
    try:
        ...
        # Get request parameters
        data = request.get_json() or {}
        force_refresh = data.get('force_refresh', False)  # Refresh even if IMVDb ID exists
        ...
        with get_db() as session:
            ...
            for video in videos:
                try:
                    ...
                    if not artist_name:
                        logger.warning(f"Skipping video {video.id}: No artist associated")
                        errors += 1
                        error_details.append({
                            'video_id': video.id,
                            'title': video_title,
                    ...
                    logger.info(f"Processing {processed}/{len(videos)}: {video_title} by {artist_name}")
                    ...
                    if imvdb_data:
                        ...
                        logger.debug(f"No IMVDb match found for: {video_title} by {artist_name}")
            ...
            logger.info(f"Metadata refresh completed: {processed} processed, {updated} updated, {errors} errors")
...
@require_rate_limit(max_requests=50, window_seconds=3600)  # Limit video updates to 50 per hour
@validate_request_data(max_payload_size=1024*50)  # 50KB max for video metadata
def update_video(video_id):
    """Update video metadata and optionally move/rename file"""
    try:
        # Validate video ID
        validated_video_id = InputValidator.validate_integer(video_id, min_val=1, max_val=999999999)
        ...
        if 'artist_name' in data and data['artist_name']:
            data['artist_name'] = InputValidator.sanitize_string(data['artist_name'], max_length=100)
        if 'video_url' in data and data['video_url']:
            if not InputValidator.validate_url(data['video_url']):
                return jsonify({'error': 'Invalid video URL format'}), 400
        if 'thumbnail_url' in data and data['thumbnail_url']:
            if not InputValidator.validate_url(data['thumbnail_url']):
                return jsonify({'error': 'Invalid thumbnail URL format'}), 400
        if 'year' in data:
            validated_year = InputValidator.validate_integer(data['year'], min_val=1900, max_val=2100)
            if data['year'] is not None and validated_year is None:
                return jsonify({'error': 'Invalid year value'}), 400
            data['year'] = validated_year
        if 'duration' in data:
            validated_duration = InputValidator.validate_integer(data['duration'], min_val=0, max_val=86400)
            if data['duration'] is not None and validated_duration is None:
                return jsonify({'error': 'Invalid duration value'}), 400
            data['duration'] = validated_duration
        if 'genres' in data and isinstance(data['genres'], list):
            data['genres'] = [InputValidator.sanitize_string(genre, max_length=50) for genre in data['genres'][:20]]  # Limit to 20 genres
        
        with get_db() as session:
            video = session.query(Video).filter(Video.id == validated_video_id).first()
            ...
            move_file = bool(data.get('move_file', False))
            
            # Update video metadata with sanitized data
            if 'title' in data and data['title']:
            ...
            if 'genres' in data and video.artist_id:
                try:
                    ...
                    genre_service._update_artist_genres_with_session(video.artist_id, session)
                except Exception as e:
                    logger.warning(f"Failed to update artist genres for video {video_id}: {e}")
            ...
            logger.info(f"Updated video {video_id}: {video.title} by {video.artist.name if video.artist else 'Unknown'}")
...
def _move_and_rename_video_file(original_path, new_artist_name, new_title, original_title, original_artist_name):
    """
    ...
    try:
        ...
        base_dir = os.path.dirname(os.path.dirname(current_file_path))  # Go up from artist folder
...
def update_video_status(video_id):
    ...
    try:
        ...
        with get_db() as session:
            ...
            old_status = video.status.value if hasattr(video.status, 'value') else video.status
            video.status = VideoStatus[new_status]
            session.commit()
            
            logger.info(f"Updated video {video_id} status from {old_status} to {new_status}")
...
def download_all_wanted_videos_internal(limit=50):
    ...
    try:
        ...
        with get_db() as session:
            wanted_videos = session.query(Video.id, Video.title, Artist.name).join(Artist).filter(
                Video.status == VideoStatus.WANTED
            ).limit(limit).all()
            ...
            try:
                with get_db() as session:
                    ...
                    if not video_url:
                        ...
                        error_reason = 'No URL available for download'
                        if video.youtube_id:
                            error_reason = 'YouTube ID found but URL resolution failed (possibly age-restricted)'
                        elif 'f616' in video_title or video_title.startswith('_'):
                            error_reason = 'Video title appears malformed - may need manual cleanup'
...
def download_all_wanted_videos():
    ...
    try:
        data = request.get_json() or {}
        limit = data.get('limit', 50)  # Default limit to prevent overwhelming the system
            ...
            for artist in artists:
                try:
                    ...
                    artist_name_variations = [
                        artist_name,
                        artist_name.replace('"', "'"),  # Replace double quotes with single quotes
                        artist_name.replace('"', ''),   # Remove quotes entirely
                        artist_name.replace("'", '"'),  # Replace single quotes with double quotes
                    ...
                    for name_variant in artist_name_variations:
                        ...
                        if potential_dir.exists():
                            ...
                            logger.info(f"Found directory for artist '{artist_name}' using variant: '{name_variant}'")
                            break
                    
                    # Check if artist directory exists
                    if not artist_dir:
                        logger.debug(f"No directory found for artist: {artist_name} (tried {len(artist_name_variations)} variations)")
                        continue
                    ...
                    logger.info(f"Scanning {len(video_files)} video files for artist: {artist_name}")
                    
                    # Get existing videos for this artist
                    existing_videos = session.query(Video).filter(Video.artist_id == artist.id).all()
                    existing_paths = {v.local_path for v in existing_videos if v.local_path}
                    ...
                    for video_file in video_files:
                        try:
                            ...
                            for video in existing_videos:
                                if (video.title.lower() == extracted_title.lower() or 
                                    video.title.lower() in extracted_title.lower() or
                                    extracted_title.lower() in video.title.lower()):
                            ...
                            video_info = {
                                ...
                                'existing_video_id': matching_video.id if matching_video else None,
                                'existing_video_title': matching_video.title if matching_video else None,
                                'existing_status': matching_video.status.value if matching_video and hasattr(matching_video.status, 'value') else (matching_video.status if matching_video else None),
                                'existing_local_path': matching_video.local_path if matching_video else None
                            }
                            ...
                            if matching_video and not dry_run:
                                old_status = matching_video.status.value if hasattr(matching_video.status, 'value') else matching_video.status
                                ...
                                logger.info(f"Updated video {matching_video.id}: {matching_video.title}")
    ...
    try:
        ...
        with get_db() as session:
            ...
            if existing_video:
                if skip_existing:
                    ...
                    if existing_video.artist_id:
                        existing_artist = session.query(Artist).filter_by(id=existing_video.artist_id).first()
                        existing_artist_name = existing_artist.name if existing_artist else None
            ...
            if not video_metadata:
                return jsonify({'error': 'Failed to extract video metadata from IMVDb'}), 500
            ...
            logger.info(f"Successfully imported video '{video_metadata['title']}' for artist '{artist_name}'")
                ...
                if download_result and download_result.get('success'):
                    ...
                    if priority != 5:  # Only update if not default priority
                        try:
                            ...
                            logger.warning(f"Could not set priority for download: {priority_error}")
                    
                    logger.info(f"Automatically started download for imported video: {video_metadata['title']} (Priority: {priority})")
                else:
                    download_error = download_result.get('error', 'Unknown download error') if download_result else 'Failed to start download'
                    logger.warning(f"Could not automatically start download for imported video {video_metadata['title']}: {download_error}")
            except Exception as e:
                download_error = str(e)
                logger.error(f"Error during automatic download for imported video {video_metadata['title']}: {e}")
        ...
        return jsonify({
            ...
            'auto_download': {
                ...
                'success': download_result.get('success', False) if download_result and auto_download else False,
            ...
            'video': {
                ...
                'artist_id': artist_id,
                'artist_name': artist_name,
                'status': 'DOWNLOADING' if download_result and download_result.get('success') else 'WANTED',
                'year': video_metadata.get('year'),
                'directors': video_metadata.get('directors', []),
                'producers': video_metadata.get('producers', []),
                'thumbnail_url': video_metadata.get('thumbnail_url'),
...
def import_video_from_youtube():
    ...
    try:
        ...
        with get_db() as session:
            ...
            # Check if video already exists
            existing_video = session.query(Video).filter_by(youtube_id=youtube_id).first()
            if existing_video:
                if skip_existing:
                    # Get existing video artist name safely
                    existing_artist_name = None
            ...
            if not title:
                ...
                try:
                    ...
                    youtube_details = youtube_search_service._get_video_details([youtube_id])
                    ...
                    logger.warning(f"Could not fetch YouTube title for video {youtube_id}: {e}")
            ...
            logger.info(f"Successfully imported YouTube video '{title}' for artist '{artist_name}'")
        ...
        if auto_download:
            try:
                ...
                if download_result and download_result.get('success'):
                    ...
                    logger.info(f"Automatically started download for imported YouTube video: {title} (Priority: {priority})")
                else:
                    download_error = download_result.get('error', 'Unknown download error') if download_result else 'Failed to start download'
                    logger.warning(f"Could not automatically start download for imported YouTube video {title}: {download_error}")
            except Exception as e:
                download_error = str(e)
                logger.error(f"Error during automatic download for imported YouTube video {title}: {e}")
        ...
        with get_db() as session:
            ...
            for download in downloads:
                queue_data.append({
                    ...
                    'artist_name': download.artist.name if download.artist else 'Unknown',
...
def update_download_priority(download_id):
    ...
    try:
        ...
        if not new_priority or not isinstance(new_priority, int) or new_priority < 1 or new_priority > 10:
            return jsonify({'error': 'Priority must be an integer between 1 (highest) and 10 (lowest)'}), 400
        ...
        with get_db() as session:
            ...
            if download.status not in ['pending', 'failed']:
                return jsonify({'error': f'Cannot modify priority for {download.status} downloads'}), 400
            
            old_priority = download.priority
            download.priority = new_priority
            session.commit()
            
            logger.info(f"Updated download {download_id} priority from {old_priority} to {new_priority}")
...
def bulk_update_priority():
    ...
    try:
        ...
        with get_db() as session:
            ...
            logger.info(f"Bulk priority update: {updated_count} updated, {skipped_count} skipped")
...
def reorder_queue():
    ...
    try:
        ...
        with get_db() as session:
            from src.database.models import Download, Artist
            
            # Get all pending and failed downloads
            downloads = session.query(Download).filter(
                Download.status.in_(['pending', 'failed'])
            ).all()
            ...
            for download in downloads:
                old_priority = download.priority
                new_priority = calculate_smart_priority(download, prioritization_rules, session)
            ...
            logger.info(f"Smart reordering complete: {reordered_count} downloads reordered")
...
def calculate_queue_statistics(session):
    ...
    priority_counts = session.query(
        Download.priority,
        func.count(Download.id)
    ).group_by(Download.priority).all()
...
def calculate_smart_priority(download, rules, session):
    ...
    if rules.get('prioritize_new_artists'):
        artist_download_count = session.query(Download).filter(
            Download.artist_id == artist.id,
    ...
    try:
        with get_db() as session:
            ...
            if download.status != 'downloading':
                return jsonify({'success': False, 'error': 'Download is not currently active'}), 400
            ...
            return jsonify({
                'success': True,
                'message': f'Download paused: {download.title}'
            })
...
def resume_download(download_id):
    ...
    try:
        with get_db() as session:
            ...
            return jsonify({
                'success': True,
                'message': f'Download resumed: {download.title}'
            })
...
def resume_all_downloads():
    ...
    try:
        with get_db() as session:
            ...
            paused_downloads = session.query(Download).filter(
                Download.status == 'paused'
            ).all()
...
def detect_duplicates():
    ...
    try:
        ...
        with get_db() as session:
            ...
            duplicate_videos = session.query(Video).filter(
            ...
            for dup_video in duplicate_videos:
                duplicates.append({
                    ...
                    'artist_name': dup_video.artist.name if dup_video.artist else None,
                    'status': dup_video.status.value if hasattr(dup_video.status, 'value') else dup_video.status,
                    'year': dup_video.year,
                    'url': dup_video.url,
            ...
            return jsonify({
                ...
                'current_video': {
                    ...
                    'artist_name': current_video.artist.name if current_video.artist else None,
                    'status': current_video.status.value if hasattr(current_video.status, 'value') else current_video.status,
...
def merge_duplicate_videos():
    ...
    try:
        ...
        with get_db() as session:
            ...
            duplicate_videos = session.query(Video).filter(Video.id.in_(duplicate_ids)).all()
            ...
            for dup_video in duplicate_videos:
                ...
                if merge_strategy == 'merge_data':
                    ...
                    if not primary_video.thumbnail_url and dup_video.thumbnail_url:
                        ...
                        merge_details['merged_data']['thumbnail_url'] = dup_video.thumbnail_url
                    ...
                    if not primary_video.local_path and dup_video.local_path:
                        ...
                        merge_details['merged_data']['local_path'] = dup_video.local_path
                ...
                if existing_video:
                    return jsonify({
                        ...
                        'duplicate_video': {
                            ...
                            'title': existing_video.title,
                            'artist_name': existing_video.artist.name if existing_video.artist else None,
                            'status': existing_video.status.value if hasattr(existing_video.status, 'value') else existing_video.status
                        },
