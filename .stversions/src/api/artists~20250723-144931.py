...
from src.database.models import Artist, Video, Download, VideoStatus
...
@require_rate_limit(max_requests=300, window_seconds=3600)  # Limit artist queries to 300 per hour
def get_artists():
    ...
    try:
        with get_db() as session:
            ...
            if search_term:
                sanitized_search = InputValidator.sanitize_string(search_term, max_length=100)
                query = query.filter(Artist.name.ilike(f'%{sanitized_search}%'))
...
def advanced_search():
    ...
    try:
        ...
        return search_optimization_service.cached_artist_search(query_params, execute_search)
...
def _execute_advanced_search(query_params):
    ...
    with get_db() as session:
        ...
        video_count_subquery = session.query(
            Video.artist_id,
            func.count(Video.id).label('video_count')
        ).group_by(Video.artist_id).subquery()
        
        # Use optimized query with index hints
        query = session.query(Artist).outerjoin(
        ...
        with get_db() as session:
            ...
            if artist.metadata:
                try:
                    ...
                    logger.warning(f"Failed to serialize metadata for artist {artist_id}: {meta_e}")
                    metadata = None
...
@require_rate_limit(max_requests=30, window_seconds=3600)  # Limit artist creation to 30 per hour
@validate_request_data(required_fields=['name'], max_payload_size=1024*5)  # 5KB max
def add_artist():
    ...
    try:
        ...
        artist_name = InputValidator.sanitize_string(data.get('name', ''), max_length=100)
        if not artist_name:
            return jsonify({'error': 'Valid artist name is required'}), 400
        
        # Sanitize other fields
        imvdb_id = data.get('imvdb_id')
        if imvdb_id and imvdb_id != '':
            imvdb_id_validated = InputValidator.validate_integer(imvdb_id, min_val=1, max_val=999999999)
            if imvdb_id_validated is None:
                return jsonify({'error': 'Invalid IMVDb ID'}), 400
            imvdb_id = imvdb_id_validated
        else:
            imvdb_id = None
        ...
        if isinstance(keywords, list):
            keywords = [InputValidator.sanitize_string(kw, max_length=50) for kw in keywords[:10]]  # Limit to 10 keywords
        else:
            keywords = []
        
        with get_db() as session:
            # Check if artist already exists
            existing = session.query(Artist).filter_by(name=artist_name).first()
            if existing:
        ...
        for artist_data in results:
            ...
            if artist_data.get('image'):
                if isinstance(artist_data['image'], dict):
                    thumbnail_url = (artist_data['image'].get('l') or 
                                   artist_data['image'].get('m') or 
                                   artist_data['image'].get('s') or
                                   artist_data['image'].get('t'))
        ...
        with get_db() as session:
            ...
            if not artist_name:
                ...
                if artist_data.get('slug'):
                    ...
                    return jsonify({'error': 'Artist name not available from IMVDb'}), 400
            ...
            if thumbnail_url:
                try:
                    ...
                    logger.warning(f"Failed to download thumbnail for {artist.name}: {e}")
    ...
    try:
        ...
        return jsonify({
            ...
            'summary': {
                ...
                'has_external_links': any(artist_preview['external_links'].values()),
    ...
    try:
        ...
        with get_db() as session:
            ...
            if results['summary']['successful_count'] > 0:
                session.commit()
                logger.info(f"Bulk import completed: {results['summary']['successful_count']} successful, "
                          f"{results['summary']['failed_count']} failed, {results['summary']['skipped_count']} skipped")
            else:
                session.rollback()
                logger.warning("Bulk import failed: no artists were successfully imported")
...
def import_artist_metadata(artist_id):
    """Import artist metadata from IMVDb to update existing artist"""
    try:
        with get_db() as session:
            artist = session.query(Artist).filter_by(id=artist_id).first()
            if not artist:
            ...
            if not artist.imvdb_id:
                return jsonify({'error': 'Artist has no IMVDb ID for metadata import'}), 400
            
            # Fetch fresh metadata from IMVDb
            artist_data = imvdb_service.get_artist(artist.imvdb_id)
            if not artist_data:
                return jsonify({'error': 'Failed to fetch artist data from IMVDb'}), 404
            
            # Update artist with fresh metadata
            if 'bio' in artist_data and artist_data['bio']:
                artist.bio = artist_data['bio']
            
            if 'genres' in artist_data and artist_data['genres']:
                artist.genres = artist_data['genres']
            
            if 'country' in artist_data and artist_data['country']:
                artist.country = artist_data['country']
            
            if 'formed_year' in artist_data and artist_data['formed_year']:
                artist.formed_year = artist_data['formed_year']
            
            if 'website' in artist_data and artist_data['website']:
                artist.website = artist_data['website']
            
            # Update thumbnail if available and current one is missing
            if 'image_url' in artist_data and artist_data['image_url']:
                if not artist.thumbnail_path or not os.path.exists(artist.thumbnail_path):
                    try:
                        thumbnail_path = thumbnail_service.download_artist_thumbnail(
                            artist.name, artist_data['image_url']
                        )
                        if thumbnail_path:
                            artist.thumbnail_path = thumbnail_path
                            artist.thumbnail_url = artist_data['image_url']
                            artist.thumbnail_source = 'imvdb'
                    except Exception as e:
                        logger.warning(f"Failed to download thumbnail for artist {artist_id}: {e}")
            
            # Store complete metadata
            artist.imvdb_metadata = artist_data
            artist.last_imvdb_update = datetime.utcnow()
            
            session.commit()
            
            logger.info(f"Successfully imported metadata for artist {artist.name} from IMVDb")
            
            return jsonify({
                'success': True,
                'message': 'Artist metadata imported successfully',
                'artist': {
                    'id': artist.id,
                    'name': artist.name,
                    'bio': artist.bio,
                    ...
                    'thumbnail_updated': bool(artist.thumbnail_path),
                    'last_updated': artist.last_imvdb_update.isoformat() if artist.last_imvdb_update else None
                }
            }), 200
    ...
    try:
        with get_db() as session:
            ...
            stats = {
                ...
                'downloaded_videos': len([v for v in videos if v.status == 'DOWNLOADED']),
                'wanted_videos': len([v for v in videos if v.status == 'WANTED']),
                'downloading_videos': len([v for v in videos if v.status == 'DOWNLOADING']),
            ...
            for video in videos:
                videos_list.append({
                    ...
                    'discovered_date': video.discovered_date.isoformat() if video.discovered_date else None,
    ...
    try:
        with get_db() as session:
            ...
            if artist_imvdb_id:
                ...
                videos_data = imvdb_service.get_artist_videos_by_id(artist_imvdb_id, limit=limit)
            else:
                logger.info(f"Using name search for video discovery for artist {artist_name}")
                videos_data = imvdb_service.search_artist_videos(artist_name, limit=limit)
            ...
            youtube_videos = []
            try:
                youtube_data = youtube_search_service.search_artist_videos(artist_name, limit=20)
                if youtube_data and youtube_data.get('videos'):
                    youtube_videos = youtube_data['videos']
                    logger.info(f"Found {len(youtube_videos)} YouTube videos for artist {artist_name}")
            except Exception as e:
            ...
            for video_data in imvdb_videos:
                ...
                if directors_filter:
                    ...
                    if not any(director.lower() in [d.lower() for d in directors_filter] 
                ...
                existing_video = session.query(Video).filter(
                    Video.imvdb_id == video_metadata['imvdb_id']
                ).first()
                ...
                video_info = {
                    ...
                    'existing_status': existing_video.status.value if existing_video else None,
                ...
                if existing_video:
                    ...
                    try:
                        ...
                        logger.error(f"Failed to import video {video_metadata['title']}: {e}")
            ...
            for youtube_video in youtube_videos:
                ...
                existing_video = session.query(Video).filter(
                    Video.youtube_id == youtube_video['youtube_id']
                ).first()
                ...
                if existing_video:
                    ...
                    try:
                        ...
                        logger.error(f"Failed to import video {youtube_video['title']}: {e}")
                        video_info['import_error'] = str(e)
            ...
            if auto_import and imported_count > 0:
                logger.info(f"Auto-imported {imported_count} videos for artist {artist_name}")
...
def get_search_filters():
    ...
    try:
        with get_db() as session:
            # Get unique keywords
            keywords_query = session.query(Artist.keywords).filter(Artist.keywords.isnot(None)).all()
            all_keywords = set()
            ...
            monitored_artists = session.query(Artist).filter(Artist.monitored == True).count()
            auto_download_artists = session.query(Artist).filter(Artist.auto_download == True).count()
            artists_with_thumbnails = session.query(Artist).filter(
                or_(Artist.thumbnail_url.isnot(None), Artist.thumbnail_path.isnot(None))
            ).count()
            artists_with_imvdb = session.query(Artist).filter(Artist.imvdb_id.isnot(None)).count()
            ...
            return jsonify({
                'filters': {
                    ...
                    'video_count_range': {
                        'min': int(video_count_stats.min_videos) if video_count_stats.min_videos else 0,
                        'max': int(video_count_stats.max_videos) if video_count_stats.max_videos else 0,
                        'average': float(video_count_stats.avg_videos) if video_count_stats.avg_videos else 0
                    }
                },
                'statistics': {
                    ...
                    'monitoring_percentage': round((monitored_artists / total_artists * 100), 2) if total_artists > 0 else 0,
                    'thumbnail_coverage': round((artists_with_thumbnails / total_artists * 100), 2) if total_artists > 0 else 0,
                    'imvdb_coverage': round((artists_with_imvdb / total_artists * 100), 2) if total_artists > 0 else 0
                }
            }), 200
    ...
    try:
        ...
        with get_db() as session:
            ...
            suggestions = session.query(Artist.name).filter(
                Artist.name.ilike(f'%{query_term}%')
            ).order_by(Artist.name).limit(limit).all()
            
            suggestion_list = [{'text': suggestion[0], 'type': 'artist_name'} for suggestion in suggestions]
            ...
            if len(suggestion_list) < limit:
                ...
                keywords_query = session.query(Artist.keywords).filter(
                    Artist.keywords.isnot(None)
                ).all()
                
                for keywords_row in keywords_query:
                    if keywords_row[0]:
                        for keyword in keywords_row[0]:
                            if keyword and isinstance(keyword, str) and query_term.lower() in keyword.lower():
                                if keyword not in [s['text'] for s in keyword_suggestions]:
                                    keyword_suggestions.append({'text': keyword, 'type': 'keyword'})
...
def search_imvdb_matches(artist_id):
    ...
    try:
        ...
        logger.info(f"IMVDb API key for {artist_name}: {'SET' if api_key else 'NOT SET'}")
        ...
        if not matches:
            return jsonify({
                'matches': [],
                'message': f'No matches found for "{artist_name}"'
            }), 200
        ...
        for match in matches:
            ...
            formatted_match = {
                ...
                'type': match.get('type'),
                'slug': match.get('slug'),
                'confidence': 'high' if (display_name and display_name.lower() == artist_name.lower()) else 'medium'
            }
    ...
    try:
        ...
        if 'image' in artist_data:
            ...
            if isinstance(image_data, dict):
                thumbnail_url = (image_data.get('o') or image_data.get('l') or 
                               image_data.get('b') or image_data.get('m') or 
                               image_data.get('s'))
        ...
        if thumbnail_url:
            try:
                ...
                if new_thumbnail_path:
                    logger.info(f"Updated thumbnail for {artist_name}: {new_thumbnail_path}")
            except Exception as e:
        ...
        with get_db() as session:
            ...
            if artist_data.get('bio') and artist_data['bio'] != current_metadata.get('bio'):
                current_metadata['bio'] = artist_data['bio']
                updated_fields.append('biography')
                metadata_updated = True
            
            if artist_data.get('birth_date') and artist_data['birth_date'] != current_metadata.get('birth_date'):
                current_metadata['birth_date'] = artist_data['birth_date']
                updated_fields.append('birth date')
                metadata_updated = True
            
            if artist_data.get('death_date') and artist_data['death_date'] != current_metadata.get('death_date'):
                current_metadata['death_date'] = artist_data['death_date']
                updated_fields.append('death date')
                metadata_updated = True
            
            if artist_data.get('origin') and artist_data['origin'] != current_metadata.get('origin'):
        ...
        logger.error(f"Failed to update metadata from match for artist {artist_id}: {e}")
        return jsonify({'error': str(e)}), 500
...
def test_thumbnail():
    ...
    try:
        ...
        placeholder_paths = [
            project_root / 'frontend/static/placeholder-artist.png',
            project_root / 'frontend/static/placeholder-video.png'
        ]
...
def populate_artist_thumbnails():
    ...
    try:
        ...
        with get_db() as session:
            ...
            artist_ids_query = session.query(Artist.id, Artist.name).filter(Artist.thumbnail_url.is_(None)).distinct()
            ...
            logger.info(f"Processing {len(artist_data_list)} artists for thumbnail population")
                    ...
                    if artist_data:
                        ...
                        if 'image' in artist_data:
                            ...
                            if isinstance(image_data, dict):
                                ...
                                thumbnail_url = (image_data.get('l') or 
                                               image_data.get('m') or 
                                               image_data.get('s') or
                                               image_data.get('o') or  # original
                                               image_data.get('b'))    # big
                            elif isinstance(image_data, str):
                                # Only use the URL if it's not just the main IMVDb site
                                if image_data and image_data != 'https://imvdb.com/':
                                    thumbnail_url = image_data
                        
                        logger.info(f"Extracted thumbnail URL for {artist_name}: {thumbnail_url}")
                        ...
                        if thumbnail_url and thumbnail_url != 'https://imvdb.com/':
                            ...
                            if thumbnail_path:
                                ...
                                try:
                                    artist_to_update = session.query(Artist).filter_by(id=artist_id).first()
                                    if artist_to_update:
                                        artist_to_update.thumbnail_url = thumbnail_url
                                        artist_to_update.thumbnail_path = thumbnail_path
                                        artist_to_update.imvdb_id = artist_data.get('id')
                                        session.flush()  # Flush immediately to catch any issues
                                        updated_count += 1
                                except Exception as update_error:
                                    logger.error(f"Failed to update artist {artist_name} in database: {update_error}")
                                    results.append({
                                        'artist_id': artist_id,
                                        'artist_name': artist_name,
                                        'status': 'db_error',
    ...
    try:
        with get_db() as session:
            ...
            if artist.thumbnail_path:
                ...
                if os.path.isabs(artist.thumbnail_path):
                    thumbnail_path = artist.thumbnail_path
                else:
                    # Resolve relative path from project root
                    thumbnail_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), artist.thumbnail_path)
                
                if os.path.exists(thumbnail_path):
                    logger.debug(f"Serving local thumbnail: {thumbnail_path}")
                    return send_file(thumbnail_path, mimetype='image/jpeg')
                else:
                    logger.warning(f"Thumbnail file not found: {thumbnail_path}")
                    logger.warning(f"Current working directory: {os.getcwd()}")
                    logger.warning(f"File exists check failed for: {artist.thumbnail_path}")
            
            # If no local thumbnail, try to download from URL if available
            if artist.thumbnail_url:
                try:
                    logger.debug(f"Attempting to download thumbnail from: {artist.thumbnail_url}")
                    ...
                    if downloaded_path and os.path.exists(downloaded_path):
                        ...
                        session.commit()
                        logger.debug(f"Downloaded and serving thumbnail: {downloaded_path}")
    ...
    try:
        ...
        with get_db() as session:
            ...
            if 'folder_path' in data:
                artist.folder_path = data['folder_path'].strip() if data['folder_path'] else None
            ...
            if 'imvdb_id' in data:
                ...
                if new_imvdb_id == '' or new_imvdb_id is None:
                    ...
                    try:
                        ...
                        if imvdb_id_int <= 0:
                            return jsonify({'error': 'IMVDb ID must be a positive integer'}), 400
                        ...
                        artist.imvdb_id = str(imvdb_id_int)
                        logger.info(f"Updated IMVDb ID for artist {artist.name} to {imvdb_id_int}")
                        
                    except (ValueError, TypeError):
                        return jsonify({'error': 'IMVDb ID must be a valid integer'}), 400
    ...
    try:
        ...
        with get_db() as session:
            ...
            if remove_thumbnail:
                ...
                return jsonify({'error': 'Either thumbnail_url or remove_thumbnail must be provided'}), 400
    ...
    try:
        ...
        try:
            ...
            if artist_data and 'image' in artist_data:
                ...
                if isinstance(image_data, dict):
                    ...
                    thumbnail_url = (image_data.get('o') or 
                                   image_data.get('l') or 
                                   image_data.get('b') or
                                   image_data.get('m') or 
                                   image_data.get('s'))
                ...
                if thumbnail_url:
                    ...
                    logger.info(f"Found thumbnail for {artist_name} from IMVDb: {thumbnail_url}")
        ...
        try:
            ...
            if thumbnail_url:
                ...
                logger.info(f"Found thumbnail for {artist_name} from Wikipedia: {thumbnail_url}")
        ...
        try:
            ...
            if youtube_thumbnail:
                ...
                logger.info(f"Found thumbnail for {artist_name} from YouTube: {youtube_thumbnail}")
...
def scan_missing_thumbnails():
    ...
    try:
        ...
        artist_data_list = []
        with get_db() as session:
            artists_without_thumbnails = session.query(Artist.id, Artist.name).filter(
                or_(
                    Artist.thumbnail_url.is_(None),
                    Artist.thumbnail_url == '',
                    Artist.thumbnail_path.is_(None),
            ...
            artist_data_list = [(artist.id, artist.name) for artist in artists_without_thumbnails]
                        ...
                        if downloaded_path and os.path.exists(downloaded_path):
                            ...
                            with get_db() as session:
                                ...
                                if artist:
                                    ...
                                    session.commit()
                                    updated_count += 1
                                    logger.info(f"Found and downloaded thumbnail for {artist_name} from {source}")
                        
                    except Exception as download_e:
                        logger.warning(f"Failed to download thumbnail for {artist_name}: {download_e}")
            
            except Exception as artist_e:
                logger.warning(f"Error processing artist {artist_name}: {artist_e}")
                continue
        
        logger.info(f"Thumbnail scan completed: {updated_count}/{missing_count} artists updated")
...
def delete_artist(artist_id):
    ...
    try:
        with get_db() as session:
            ...
            video_count = session.query(Video).filter_by(artist_id=artist_id).count()
            download_count = session.query(Download).join(Video).filter(Video.artist_id == artist_id).count()
            
            # Check if force delete is requested
            force_delete = request.args.get('force', 'false').lower() in ['true', '1', 'yes']
            ...
            if force_delete:
                ...
                video_records = session.query(Video).filter_by(artist_id=artist_id).all()
                ...
                if video_ids:
                    ...
                    for video_id in video_ids:
                        download_count = session.query(Download).filter_by(video_id=video_id).delete()
                ...
                logger.info(f"Force delete: removed {videos_deleted} videos and {downloads_deleted} downloads")
...
def merge_artists(artist_id):
    ...
    try:
        ...
        with get_db() as session:
            ...
            source_videos = session.query(Video).filter_by(artist_id=source_artist.id).all()
            source_downloads = session.query(Download).filter_by(artist_id=source_artist.id).all()
            ...
            if (source_artist.thumbnail_path and 
                source_artist.thumbnail_path != target_artist.thumbnail_path and
                os.path.exists(source_artist.thumbnail_path)):
                try:
                    os.remove(source_artist.thumbnail_path)
                    logger.info(f"Removed source artist thumbnail: {source_artist.thumbnail_path}")
                except Exception as e:
            ...
            logger.info(f"Merged artist '{source_artist_name}' into '{target_artist.name}': "
...
def update_artist_metadata_from_imvdb(artist_id):
    ...
    try:
        ...
        if existing_imvdb_id:
            ...
            if not artist_data:
                logger.warning(f"Artist not found by existing IMVDb ID {existing_imvdb_id}, trying name search")
        ...
        if 'image' in artist_data:
            ...
            if isinstance(image_data, dict):
                ...
                thumbnail_url = (image_data.get('o') or 
                               ...
                               image_data.get('s'))
                if thumbnail_url:
                    logger.debug(f"Found valid thumbnail URL from IMVDb image dict: {thumbnail_url}")
                else:
                    logger.debug(f"No valid thumbnail URLs found in IMVDb image dict: {image_data}")
            elif isinstance(image_data, str) and image_data != 'https://imvdb.com/':
                thumbnail_url = image_data
                ...
                logger.debug(f"Unknown image data format from IMVDb: {type(image_data)}")
        else:
        ...
        if thumbnail_url and thumbnail_url != existing_thumbnail_url:
            try:
                logger.info(f"Downloading new thumbnail for {artist_name}: {thumbnail_url}")
                downloaded_path = thumbnail_service.download_artist_thumbnail(
                    artist_name, 
                    thumbnail_url
                )
                ...
                if downloaded_path and os.path.exists(downloaded_path):
                    # Remove old thumbnail if exists
                    if existing_thumbnail_path and os.path.exists(existing_thumbnail_path):
                        try:
                ...
                thumbnail_url = existing_thumbnail_url  # Keep existing URL if download failed
        elif thumbnail_url == existing_thumbnail_url:
            logger.debug(f"Thumbnail URL unchanged for {artist_name}: {thumbnail_url}")
        else:
            logger.info(f"No valid thumbnail URL available for {artist_name} from IMVDb, trying Wikipedia fallback")
            ...
            try:
                ...
                if wiki_thumbnail_url and wiki_thumbnail_url != existing_thumbnail_url:
                    try:
                        logger.info(f"Downloading Wikipedia thumbnail for {artist_name}: {wiki_thumbnail_url}")
                        downloaded_path = thumbnail_service.download_artist_thumbnail(
                            artist_name, 
                            wiki_thumbnail_url
                        )
                        ...
                        if downloaded_path and os.path.exists(downloaded_path):
                            ...
                            logger.info(f"Successfully updated thumbnail for {artist_name} from Wikipedia")
                        else:
                            logger.warning(f"Failed to download Wikipedia thumbnail for {artist_name}")
                    except Exception as e:
                        logger.warning(f"Failed to download Wikipedia thumbnail: {e}")
                elif wiki_thumbnail_url == existing_thumbnail_url:
                    logger.debug(f"Wikipedia thumbnail URL unchanged for {artist_name}: {wiki_thumbnail_url}")
                else:
        ...
        with get_db() as session:
            ...
            try:
                ...
                if 'imvdb_id' in str(e) and 'Duplicate entry' in str(e):
                    ...
                    conflicting_artist = session.query(Artist).filter_by(
            ...
            logger.info(f"Updated metadata for artist {artist_name}, fields: {updated_fields}")
            ...
            return jsonify({
                ...
                'imvdb_data': {
                    ...
                    'genres': artist_data.get('genres', []),
                    'verified': artist_data.get('verified', False),
                    'artist_video_count': artist_data.get('artist_video_count', 0),
                    'featured_video_count': artist_data.get('featured_video_count', 0)
                }
            }), 200
...
@require_rate_limit(max_requests=10, window_seconds=3600)  # Limit bulk artist deletes to 10 per hour
@validate_request_data(required_fields=['artist_ids'], max_payload_size=1024*5)  # 5KB max
def bulk_delete_artists():
    ...
    try:
        ...
        # Validate and sanitize artist IDs
        sanitized_ids = []
        for artist_id in artist_ids:
            validated_id = InputValidator.validate_integer(artist_id, min_val=1, max_val=999999999)
            if validated_id is None:
                return jsonify({'error': f'Invalid artist ID: {artist_id}'}), 400
            sanitized_ids.append(validated_id)
        
        # Limit bulk operations to reasonable size
        if len(sanitized_ids) > 50:
            return jsonify({'error': 'Bulk delete limited to 50 artists at once'}), 400
        ...
        with get_db() as session:
            ...
            artists_to_delete = session.query(Artist).filter(Artist.id.in_(sanitized_ids)).all()
            ...
            for artist in artists_to_delete:
                ...
                try:
                    ...
                    logger.error(f"Failed to delete artist {artist_name} (ID: {artist_id}): {e}")
...
def cleanup_artists_with_zero_videos():
    ...
    try:
        with get_db() as session:
            ...
            artists_with_zero_videos = session.query(Artist) \
                .join(video_count_subquery, Artist.id == video_count_subquery.c.artist_id) \
            ...
            for artist in artists_with_zero_videos:
                try:
                    ...
                    logger.error(f"Failed to delete artist {getattr(artist, 'name', 'Unknown')}: {e}")
                    continue
...
def merge_multiple_artists():
    ...
    try:
        data = request.get_json()
        if not data or 'primary_artist_id' not in data or 'secondary_artist_ids' not in data:
            return jsonify({'error': 'primary_artist_id and secondary_artist_ids required'}), 400
        ...
        if not isinstance(secondary_artist_ids, list) or not secondary_artist_ids:
            return jsonify({'error': 'secondary_artist_ids must be a non-empty list'}), 400
        
        if primary_artist_id in secondary_artist_ids:
            return jsonify({'error': 'primary_artist_id cannot be in secondary_artist_ids'}), 400
        
        with get_db() as session:
            # Get primary artist
            primary_artist = session.query(Artist).filter_by(id=primary_artist_id).first()
            if not primary_artist:
                return jsonify({'error': 'Primary artist not found'}), 404
            
            # Get secondary artists
            secondary_artists = session.query(Artist).filter(Artist.id.in_(secondary_artist_ids)).all()
            if len(secondary_artists) != len(secondary_artist_ids):
                return jsonify({'error': 'One or more secondary artists not found'}), 404
            ...
            for secondary_artist in secondary_artists:
                try:
                    ...
                    if secondary_artist.imvdb_id:
                        ...
                        logger.info(f"Cleared IMVDb ID {secondary_imvdb_id} from secondary artist {secondary_artist.name}")
                        ...
                        if not primary_artist.imvdb_id:
                            ...
                            logger.info(f"Assigned IMVDb ID {secondary_imvdb_id} to primary artist {primary_artist.name}")
                    
                    # Move videos from secondary to primary
                    session.query(Video).filter_by(artist_id=secondary_artist.id).update({
                        'artist_id': primary_artist_id
                    })
                    
                    # Move downloads from secondary to primary
                    session.query(Download).filter_by(artist_id=secondary_artist.id).update({
                        'artist_id': primary_artist_id
                    })
                    ...
                    if secondary_artist.keywords:
                        ...
                        merged_keywords = list(primary_keywords.union(secondary_keywords))
                        primary_artist.keywords = merged_keywords
                    
                    # Keep the better thumbnail if primary doesn't have one
                    if not primary_artist.thumbnail_url and secondary_artist.thumbnail_url:
                    ...
                    if secondary_artist.auto_download and not primary_artist.auto_download:
                    ...
                    merged_count += 1
                    logger.info(f"Merged artist {secondary_artist.name} into {primary_artist.name}")
...
@require_rate_limit(max_requests=20, window_seconds=3600)  # Limit thumbnail uploads to 20 per hour
def upload_artist_thumbnail(artist_id):
    """Upload a manual thumbnail for an artist"""
    try:
        # Validate artist ID
        validated_artist_id = InputValidator.validate_integer(artist_id, min_val=1, max_val=999999999)
        if validated_artist_id is None:
            return jsonify({'error': 'Invalid artist ID'}), 400
            
        with get_db() as session:
            artist = session.query(Artist).filter_by(id=validated_artist_id).first()
            if not artist:
                return jsonify({'error': 'Artist not found'}), 404
            ...
            # Additional security: Validate file content (basic image header check)
            file_data = file.read()
            ...
            if old_thumbnail_path and old_thumbnail_metadata:
                thumbnail_service.delete_thumbnail_files(old_thumbnail_path, old_thumbnail_metadata)
            ...
            logger.info(f"Successfully uploaded manual thumbnail for artist {artist.name}")
...
def crop_artist_thumbnail(artist_id):
    ...
    try:
        ...
        if not isinstance(crop_box, list) or len(crop_box) != 4:
            return jsonify({'error': 'crop_box must be [left, top, right, bottom]'}), 400
    ...
    try:
        with get_db() as session:
            ...
            thumbnail_info = {
                ...
                'thumbnail_uploaded_at': artist.thumbnail_uploaded_at.isoformat() if artist.thumbnail_uploaded_at else None,
...
def get_artist_thumbnail_size(artist_id, size):
    ...
    try:
        ...
        with get_db() as session:
            ...
            if artist.thumbnail_path and os.path.exists(artist.thumbnail_path):
                logger.debug(f"Serving primary thumbnail as {size}: {artist.thumbnail_path}")
            ...
            if artist.thumbnail_url:
                try:
                    ...
                    if downloaded_path:
                        artist.thumbnail_path = downloaded_path
                        artist.thumbnail_source = 'imvdb'  # Assume IMVDb for URL downloads
                        session.commit()
                        logger.debug(f"Downloaded and serving thumbnail: {downloaded_path}")
                        return send_file(downloaded_path, mimetype='image/jpeg')
...
def get_thumbnail_statistics():
    ...
    try:
        ...
        with get_db() as session:
            ...
            artists_by_source = {
                'imvdb': session.query(Artist).filter(Artist.thumbnail_source == 'imvdb').count(),
                'wikipedia': session.query(Artist).filter(Artist.thumbnail_source == 'wikipedia').count(),
                'manual': session.query(Artist).filter(Artist.thumbnail_source == 'manual').count(),
                'manual_crop': session.query(Artist).filter(Artist.thumbnail_source == 'manual_crop').count(),
            }
            
            total_videos = session.query(Video).count()
            videos_with_thumbnails = session.query(Video).filter(
                or_(Video.thumbnail_url.isnot(None), Video.thumbnail_path.isnot(None))
            ).count()
            ...
            database_stats = {
                'artists': {
                    ...
                    'coverage_percentage': round((artists_with_thumbnails / total_artists * 100), 2) if total_artists > 0 else 0,
                    'by_source': artists_by_source
                },
                'videos': {
                    'total': total_videos,
                    'with_thumbnails': videos_with_thumbnails,
                    'coverage_percentage': round((videos_with_thumbnails / total_videos * 100), 2) if total_videos > 0 else 0
                }
            }
