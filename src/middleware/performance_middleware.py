...
class PerformanceTrackingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, exclude_paths: Optional[list] = None):
        ...
        logger.info(f"📊 Performance tracking middleware initialized, excluding: {self.exclude_paths}")
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if response_time > 1000:
                logger.warning(f"🐌 Slow API response: {endpoint} took {response_time:.1f}ms (status: {status_code})")
            ...
            logger.error(f"❌ API request failed: {endpoint} after {response_time:.1f}ms - {e}")
            raise
...
class CacheHeadersMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, default_cache_ttl: int = 300):
        ...
        logger.info(f"🗄️ Cache headers middleware initialized with {default_cache_ttl}s default TTL")
...
class ResourceMonitoringMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, track_memory: bool = True):
        ...
        logger.info(f"📈 Resource monitoring middleware initialized (memory tracking: {track_memory})")
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if self.track_memory and initial_memory:
                try:
                    ...
                    if memory_delta > 50 * 1024 * 1024:
                        logger.warning(f"🧠 High memory usage: {path} used {memory_delta / (1024*1024):.1f}MB")
            ...
            processing_time = time.time() - start_time
            logger.error(f"❌ Resource monitoring error for {path}: {e} (after {processing_time:.3f}s)")
            raise