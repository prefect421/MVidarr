...
from src.api.protected_endpoints import apply_authentication_protection, create_endpoint_protection_report
    ...
    def get_authentication_status(self):
        ...
        try:
            ...
            try:
                ...
                status['oauth_providers'] = list(oauth_service.get_available_providers().keys())
            except Exception as e:
        ...
        def apply_protection_once():
            ...
            if not protection_applied:
                ...
                try:
                    ...
                    logger.info(f"  - Protected: {report['summary']['protected_endpoints']}")
                    logger.info(f"  - Public: {report['summary']['public_endpoints']}")
                    logger.info(f"  - Coverage: {report['summary']['protection_coverage']}%")
                    
                    if report['unprotected_endpoints']:
                        logger.warning(f"  - Unprotected: {len(report['unprotected_endpoints'])} endpoints")
                        for endpoint in report['unprotected_endpoints'][:5]:  # Log first 5
                            logger.warning(f"    * {endpoint['endpoint']} {endpoint['methods']}")
...
def init_authentication(app: Flask):
    ...
    try:
        ...
        if success:
            ...
            logger.info(f"   - OAuth Providers: {', '.join(status.get('oauth_providers', []))}")
        ...
        try:
            ...
            health_data = {
                ...
                'protection': {
                    ...
                    'coverage': protection['summary']['protection_coverage'] if protection else 0
                },
            ...
            is_healthy = (
                ...
                status.get('default_admin_exists', False) and
