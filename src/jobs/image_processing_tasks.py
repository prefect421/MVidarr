...
class ImageProcessingTask:
    ...
    def update_progress(self, progress: float, status_message: str = ""):
        ...
        logger.debug(f"⏳ Task {self.task_id} progress: {self.progress:.1f}% - {status_message}")
    ...
    async def setup(self):
        ...
        logger.info(f"🖼️ Image processing task {self.task_id} initialized with {thread_count} threads")
...
class BulkThumbnailGenerationTask(ImageProcessingTask):
    ...
    async def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"🖼️ Generating {len(jobs)} thumbnails using {self.stats.thread_pool_size} threads")
            ...
            logger.info(f"✅ Bulk thumbnail generation completed: {self.stats.processed_images}/{len(jobs)} successful")
            return result
...
class ConcurrentImageOptimizationTask(ImageProcessingTask):
    ...
    async def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"🖼️ Optimizing {len(valid_sources)} images using {self.stats.thread_pool_size} threads")
            ...
            logger.info(f"✅ Image optimization completed: {self.stats.processed_images}/{len(valid_sources)} successful")
            return result
...
class ImageAnalysisTask(ImageProcessingTask):
    ...
    async def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"🔍 Analyzing {len(valid_sources)} images using {self.stats.thread_pool_size} threads")
            ...
            logger.info(f"✅ Image analysis completed: {self.stats.processed_images}/{len(valid_sources)} successful")
            return result
