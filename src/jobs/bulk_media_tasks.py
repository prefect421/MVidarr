...
class BulkMediaProcessor:
    ...
    def __init__(self, max_workers: Optional[int] = None):
        ...
        logger.info(f"🚀 Bulk media processor initialized with {self.max_workers} workers")
    ...
    async def bulk_metadata_enrichment(
        ...
        try:
            ...
            logger.info(f"✅ Metadata enrichment completed: {progress.successful_items}/{progress.total_items} successful")
    ...
    async def _process_metadata_batch(
        ...
        if image_items:
            ...
            logger.info(f"📊 Cache performance: {cache_hits}/{len(image_items)} hits ({cache_hits/len(image_items)*100:.1f}%)")
        ...
        logger.info(f"📊 Batch processed: {len(batch)} items in {processing_time:.2f}s ({len(batch)/processing_time:.1f} items/sec)")
    ...
    def _extract_image_metadata(self, item: BulkMediaItem) -> Optional[ImageMetadata]:
        ...
        try:
            ...
            logger.warning(f"⚠️ Failed to extract image metadata for {item.file_path}: {e}")
            return None
    ...
    async def bulk_collection_import(
        ...
        try:
            ...
            logger.info(f"✅ Collection import completed: {progress.successful_items}/{progress.total_items} files imported")
    ...
    async def bulk_cleanup_operation(
        ...
        try:
            ...
            logger.info(f"✅ Cleanup operation completed: {progress.successful_items} items processed")
