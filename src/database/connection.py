        ...
        try:
            ...
            with temp_engine.connect() as connection:
                ...
                result = connection.execute(
                    text(f"SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = '{self.config.DB_NAME}'")
                )
                
                if not result.fetchone():
                    # Create database
                    connection.execute(text(f"CREATE DATABASE {self.config.DB_NAME} CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci"))
