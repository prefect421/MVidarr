...
class Artist(Base):
    ...
    thumbnail_source = Column(String(50), nullable=True)  # 'imvdb', 'wikipedia', 'manual', 'generated'
    ...
    genres = Column(JSON, nullable=True)  # Artist genres (automatically updated from videos)
    monitored = Column(Boolean, default=True)
    source = Column(String(50), nullable=True)  # Source: 'imvdb', 'spotify_import', 'manual', etc.
    ...
    downloads = relationship("Download", back_populates="artist", cascade="all, delete-orphan")
...
class Video(Base):
    ...
    playlist_id = Column(String(100), nullable=True)  # YouTube playlist ID if from playlist
    playlist_position = Column(Integer, nullable=True)  # Position in playlist
    source = Column(String(50), nullable=True)  # Source: 'imvdb', 'youtube_playlist', 'manual', etc.
    thumbnail_url = Column(String(500), nullable=True)
    thumbnail_path = Column(String(500), nullable=True)
    thumbnail_source = Column(String(50), nullable=True)  # 'imvdb', 'youtube', 'manual', 'generated'
    thumbnail_metadata = Column(JSON, nullable=True)  # Store sizes, upload info, etc.
    thumbnail_uploaded_at = Column(DateTime, nullable=True)  # When manually uploaded
    ...
    downloads = relationship("Download", back_populates="video", cascade="all, delete-orphan")
...
class Download(Base):
    ...
    priority = Column(Integer, default=5)  # 1-10, lower is higher priority (1=highest, 10=lowest)
    ...
    __table_args__ = (
        ...
        Index('idx_download_priority_status', 'priority', 'status'),  # Composite index for queue ordering
        {'extend_existing': True}
    )
...
class PlaylistMonitor(Base):
    ...
    id = Column(Integer, primary_key=True)
    playlist_id = Column(String(100), unique=True, nullable=False)  # YouTube playlist ID
    ...
    def __repr__(self):
        return f"<PlaylistMonitor(name='{self.name}', playlist_id='{self.playlist_id}')>"