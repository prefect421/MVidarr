...
def test_lidarr_connection():
    ...
    try:
        ...
        response = requests.get(f'{server_url}/api/v1/system/status', headers=headers, timeout=10)
...
def sync_lidarr_library():
    ...
    try:
        ...
        headers = {'X-Api-Key': api_key}
        response = requests.get(f'{server_url}/api/v1/artist', headers=headers, timeout=30)
        ...
        with get_db() as session:
            for artist in artists:
                try:
                    ...
                    existing_artist = session.query(Artist).filter_by(name=artist['artistName']).first()
                    ...
                    logger.error(f"Error processing artist {artist.get('artistName', 'unknown')}: {e}")
                    continue
            ...
            for artist in monitored_artists:
                try:
                    ...
                    if not existing_artist:
                        ...
                        session.add(new_artist)
                        imported_count += 1
                        logger.info(f"Imported monitored artist from Lidarr: {artist['artistName']}")
                    else:
                    ...
                    logger.error(f"Error importing artist {artist.get('artistName', 'unknown')}: {e}")
                    continue
...
def sync_lidarr_albums():
    ...
    try:
        ...
        headers = {'X-Api-Key': api_key}
        response = requests.get(f'{server_url}/api/v1/album', headers=headers, timeout=30)
        ...
        for album in albums:
            try:
                ...
                artist_response = requests.get(f'{server_url}/api/v1/artist/{album["artistId"]}', headers=headers, timeout=10)
                if artist_response.status_code == 200:
                    ...
                    with get_db() as session:
                        existing_artist = session.query(Artist).filter_by(name=artist_name).first()
                        
                        if existing_artist:
                            # Log album information for potential video discovery
                            logger.info(f"Lidarr album sync: {artist_name} - {album['title']} ({album.get('releaseDate', 'unknown date')})")
...
def get_wanted_albums():
    ...
    try:
        ...
        headers = {'X-Api-Key': api_key}
        response = requests.get(f'{server_url}/api/v1/wanted/missing', headers=headers, timeout=30)
        ...
        for album in data.get('records', []):
            try:
                ...
                logger.error(f"Error processing wanted album {album.get('title', 'unknown')}: {e}")
                continue
