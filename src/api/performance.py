...
from src.utils.performance_monitor import get_performance_report, log_performance_summary
    ...
    try:
        ...
        if threshold:
            report["filtered_endpoints"] = [
                endpoint for endpoint in report["all_endpoints"]
...
def get_slow_endpoints():
    ...
    try:
        ...
        return jsonify({
            ...
            "analysis": {
                "critical_endpoints": [e for e in slow_endpoints if e["avg_response_time"] > 1.0],
                "needs_attention": [e for e in slow_endpoints if 0.5 < e["avg_response_time"] <= 1.0],
                "minor_issues": [e for e in slow_endpoints if e["avg_response_time"] <= 0.5],
            }
        }), 200
...
def get_performance_summary():
    ...
    try:
        ...
        if summary["slow_endpoints_count"] > 0:
            recommendations.append(f"Consider optimizing {summary['slow_endpoints_count']} slow endpoints")
        
        if summary["overall_avg_response_time"] > 1.0:
            recommendations.append("Overall API performance is slow - review database queries and caching")
        elif summary["overall_avg_response_time"] > 0.5:
            recommendations.append("API performance is acceptable but could be improved")
        else:
...
def performance_health():
    ...
    try:
        ...
        elif report["summary"]["slow_endpoints_count"] > report["summary"]["total_endpoints_monitored"] / 2:
            status = "degraded"
        ...
        return jsonify({
            "status": status,
            "monitored_endpoints": report["summary"]["total_endpoints_monitored"],
