...
def upload_certificates():
    ...
    try:
        ...
        is_valid, error, cert_info = validate_certificate_file(cert_content)
        if not is_valid:
            return jsonify({"success": False, "error": f"Certificate validation failed: {error}"}), 400
        
        # Read and validate private key
        key_content = key_file.read()
        is_valid, error = validate_private_key_file(key_content)
        if not is_valid:
            return jsonify({"success": False, "error": f"Private key validation failed: {error}"}), 400
        
        # Backup existing files if they exist
        backup_suffix = datetime.now().strftime("%Y%m%d_%H%M%S")
        if CERT_FILE.exists():
            shutil.copy2(CERT_FILE, CERT_DIR / f"certificate_backup_{backup_suffix}.crt")
        if KEY_FILE.exists():
        ...
        logger.info(f"SSL certificates uploaded successfully - expires: {cert_info['not_after']}")
...
def validate_certificate():
    ...
    try:
        if not CERT_FILE.exists():
            return jsonify({"success": False, "error": "No certificate file found"}), 404
        ...
        if is_valid:
            ...
            if cert_info["days_until_expiry"] < 30:
                validation_result["warnings"] = [f"Certificate expires in {cert_info['days_until_expiry']} days"]
            ...
            if KEY_FILE.exists():
                try:
                    ...
                    if not key_valid:
                        validation_result["errors"].append(f"Private key issue: {key_error}")
                        validation_result["valid"] = False
                except Exception as e:
                    validation_result["errors"].append(f"Error reading private key: {str(e)}")
                    validation_result["valid"] = False
            else:
...
def remove_certificates():
    ...
    try:
        ...
        if not removed_files:
            return jsonify({"success": False, "error": "No certificate files found to remove"}), 404
