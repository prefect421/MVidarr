        ...
        with get_db() as session:
            ...
            if 'timezone' in data:
                ...
                if db_user.preferences:
                    try:
                        current_prefs = json.loads(db_user.preferences) if isinstance(db_user.preferences, str) else db_user.preferences
                    except (json.JSONDecodeError, TypeError):
                        current_prefs = {}
...
def change_password():
    ...
    try:
        ...
        AuthService.revoke_all_user_sessions_except_current(user.id, request.headers.get('Authorization', ''))
