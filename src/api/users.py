...
def list_users():
    ...
    try:
        ...
        if current_user.can_manage_users():
            ...
            return jsonify(
                {"users": users_data, "total": len(users_data), "can_manage": True}
            )
        else:
            ...
            return jsonify(
                {"users": users_data, "total": len(users_data), "can_manage": False}
            )
    ...
    try:
        ...
        if success:
            AuditService.log_admin_action(
                ...
                additional_data={"new_user_role": role.value, "new_user_email": email},
            )

            return (
                jsonify({"success": True, "message": message, "user": user.to_dict()}),
                201,
            )
        else:
...
def get_user(user_id):
    ...
    try:
        ...
        with get_db() as session:
            ...
            include_sensitive = (
                current_user.can_manage_users() or user_id == current_user.id
            )
            return jsonify({"user": user.to_dict(include_sensitive=include_sensitive)})
...
def update_user_role(user_id):
    ...
    try:
        ...
        success, message = AuthService.update_user_role(
            user_id, new_role, admin_user.id
        )
...
def activate_user(user_id):
    ...
    try:
        ...
        with get_db() as session:
            ...
            AuditService.log_admin_action(
                "activate_user", target_user=user, admin_user=admin_user
            )
...
def unlock_user(user_id):
    ...
    try:
        ...
        with get_db() as session:
            ...
            AuditService.log_admin_action(
                "unlock_user", target_user=user, admin_user=admin_user
            )

            return jsonify(
                {"success": True, "message": "User account unlocked successfully"}
            )
        ...
        with get_db() as session:
            ...
            for user_session in sessions:
                ...
                session_data["is_current"] = (
                    user_session.session_token == current_session_token
                )
                sessions_data.append(session_data)

            return jsonify({"sessions": sessions_data, "total": len(sessions_data)})
...
def revoke_all_user_sessions(user_id):
    ...
    try:
        ...
        if success:
            ...
            response_data = {"success": True, "message": "All sessions revoked"}
