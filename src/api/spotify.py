    ...
    try:
        ...
        for track_item in tracks_data.get('items', []):
            ...
            formatted_tracks.append({
                'id': track.get('id'),
                'name': track.get('name'),
                'artists': [{'name': a.get('name'), 'id': a.get('id')} for a in track.get('artists', [])],
                'album': {
...
def get_status():
    ...
    try:
        ...
        has_access_token = (
            'spotify_access_token' in session or 
            spotify_service.access_token is not None
        )
        
        # Check if client credentials are configured
        configured = (
            spotify_service.client_id is not None and 
            spotify_service.client_secret is not None
        )
        ...
        return jsonify({
            ...
            'profile': profile,
            'client_id': spotify_service.client_id[:8] + '...' if spotify_service.client_id else None,
...
def authorize_spotify():
    ...
    try:
        ...
        auth_url = spotify_service.get_auth_url()
        logger.info(f"Generated Spotify authorization URL with redirect URI: {spotify_service.redirect_uri}")
