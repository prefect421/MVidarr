...
def update_user_role_web(user_id):
    ...
    try:
        ...
        success, message = AuthService.update_user_role(
            user_id, new_role, request.current_user.id
        )
    ...
    try:
        ...
        with get_db() as session:
            ...
            if user.is_active:
                ...
                AuditService.log_admin_action(
                    "activate_user", target_user=user, admin_user=request.current_user
                )
...
def restart_application():
    ...
    try:
        ...
        logger.warning(
            f"Application restart initiated by admin user: {request.current_user.username}"
        )
            ...
            try:
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        subprocess.run(
                            ["sudo", "systemctl", "restart", "mvidarr"], timeout=10
                        )
                        return
                except (
