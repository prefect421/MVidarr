...
from src.utils.auth_decorators import (
    login_required, admin_required, manager_required, user_required,
    role_required, check_content_permissions, log_user_action
)
...
def apply_authentication_protection(app):
    ...
    for rule, endpoint in endpoints:
        ...
        # MANAGER ENDPOINTS (Manager role or higher required)
        elif any(pattern in rule_str for pattern in [
            ...
            endpoint = manager_required(log_user_action(f"modified content via {rule_str}")(endpoint))
            protection_applied = True
...
def get_endpoint_protection_summary(app):
    ...
    for rule in app.url_map.iter_rules():
        if rule.endpoint in app.view_functions:
            ...
            public_patterns = [
                '/auth/', '/health', '/api/health', '/static/', '/favicon.ico'
            ]
    ...
    report = {
        'summary': {
            ...
            'protection_coverage': round(
                ...
                summary['total_endpoints'] * 100, 2
            ) if summary['total_endpoints'] > 0 else 0
        },
