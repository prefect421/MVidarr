...
def setup_page():
    ...
    try:
        ...
        if user.two_factor_enabled:
            flash(
                "Two-factor authentication is already enabled for your account.", "info"
            )
...
def initiate_setup():
    ...
    try:
        ...
        if user.two_factor_enabled:
            return (
                jsonify({"error": "Two-factor authentication is already enabled"}),
                400,
            )
...
def regenerate_backup_codes():
    ...
    try:
        ...
        if not data or "password" not in data:
            return (
                jsonify({"error": "Password is required to regenerate backup codes"}),
                400,
            )
        ...
        if success:
            return jsonify(
                {"success": True, "message": message, "backup_codes": new_codes}
            )
        else:
            return jsonify({"error": message}), 400
...
def verify_login():
    ...
    try:
        ...
        if not data or "user_id" not in data or "token" not in data:
            return (
                jsonify({"error": "User ID and verification token are required"}),
                400,
            )
...
def admin_disable_two_factor(user_id):
    ...
    try:
        ...
        success, message = TwoFactorService.disable_two_factor(
            user_id, password=None, admin_user_id=admin_user.id
        )
...
def admin_get_user_status(user_id):
    ...
    try:
        ...
        return jsonify({"success": True, "user": user_info, "status": status})
...
def admin_overview():
    ...
    try:
        ...
        with get_db() as session:
            ...
            enabled_users = (
                session.query(User).filter_by(two_factor_enabled=True).count()
            )
            pending_setup = (
                session.query(User)
                .filter(
                    User.two_factor_secret.isnot(None), User.two_factor_enabled == False
                )
                .count()
            )

            users_with_2fa = (
                session.query(User).filter_by(two_factor_enabled=True).all()
            )
        ...
        stats = {
            ...
            "enabled_percentage": round(
                (enabled_users / total_users * 100) if total_users > 0 else 0, 1
            ),
        }

        return render_template(
            "admin/2fa_overview.html", stats=stats, users_with_2fa=users_with_2fa
        )
