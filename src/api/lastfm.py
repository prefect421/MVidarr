...
def import_top_artists():
    ...
    try:
        ...
        results = lastfm_service.import_top_artists(username, period, limit, min_playcount)
...
def get_status():
    ...
    try:
        ...
        has_session = (
            session.get('lastfm_session_key') is not None or
            lastfm_service.session_key is not None
        )
        
        # Check if API credentials are configured
        configured = (
            lastfm_service.api_key is not None and
        ...
        return jsonify({
            ...
            'profile': profile,
            'api_key': lastfm_service.api_key[:8] + '...' if lastfm_service.api_key else None
        }), 200
...
def test_lastfm_integration():
    ...
    try:
        ...
        try:
            result = lastfm_service.call_api('artist.search', {
                'artist': 'test',
                'limit': 1
            })
