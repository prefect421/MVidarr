...
enhanced_discovery_bp = Blueprint(
    "enhanced_discovery", __name__, url_prefix="/enhanced_discovery"
)
...
class EnrichmentRequestSchema(Schema):
    ...
    artist_ids = fields.List(
        fields.Int(), required=True, validate=lambda x: len(x) <= 50
    )
    ...
    try:
        ...
        for artist_metadata in discovered_artists:
            result = {
                ...
                "similar_artists": artist_metadata.similar_artists[
                    :10
                ],  # Limit similar artists
...
def enrich_artists():
    ...
    try:
        ...
        for artist_id in artist_ids:
            try:
                success = enhanced_artist_discovery_service.enrich_artist_metadata(
                    artist_id
                )
...
def detect_duplicate_artists():
    ...
    try:
        ...
        for duplicate in duplicates:
            ...
            with get_db() as db:
                artist1 = (
                    db.query(Artist).filter(Artist.id == duplicate.artist_id).first()
                )
                artist2 = (
                    db.query(Artist).filter(Artist.id == duplicate.candidate_id).first()
                )
...
def get_artist_recommendations():
    ...
    try:
        ...
        logger.info(
            f"Getting recommendations for artist '{artist.name}' (ID: {artist_id})"
        )
...
def get_discovery_stats():
    ...
    try:
        ...
        with get_db() as db:
            ...
            artists_with_genres = (
                db.query(Artist)
                .filter(Artist.genres.isnot(None))
                .filter(Artist.genres != "")
                .count()
            )
            artists_with_biography = (
                db.query(Artist)
                .filter(Artist.biography.isnot(None))
                .filter(Artist.biography != "")
                .count()
            )
            artists_with_country = (
                db.query(Artist)
                .filter(Artist.country.isnot(None))
                .filter(Artist.country != "")
                .count()
            )

            # Calculate completeness percentages
            genre_completeness = (
                (artists_with_genres / total_artists * 100) if total_artists > 0 else 0
            )
            biography_completeness = (
                (artists_with_biography / total_artists * 100)
                if total_artists > 0
                else 0
            )
            country_completeness = (
                (artists_with_country / total_artists * 100) if total_artists > 0 else 0
            )

            # Overall metadata quality score
            overall_completeness = (
                genre_completeness + biography_completeness + country_completeness
            ) / 3
            ...
            stats = {
                ...
                "discovery_sources": {
                    "imvdb": {
                        "enabled": True,
                        "description": "Internet Music Video Database",
                    },
                    "spotify": {
                        "enabled": True,
                        "description": "Spotify Music Platform",
                    },
