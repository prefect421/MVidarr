...
def optimize_thumbnails():
    ...
    try:
        ...
        results = thumbnail_optimization_service.optimize_existing_thumbnails(dry_run=dry_run)
        
        message = f"Thumbnail optimization {'analysis' if dry_run else 'completed'}: " \
...
def cleanup_duplicate_thumbnails():
    ...
    try:
        ...
        message = f"Duplicate cleanup completed: {results['duplicates_found']} duplicates removed, " \
                 f"{results['space_saved']} bytes saved"
    ...
    try:
        ...
        status = {
            ...
            'thumbnail_optimization': {
                ...
                'webp_percentage': (
                    thumbnail_analysis.get('by_format', {}).get('.webp', {}).get('files', 0) / 
                    max(thumbnail_analysis.get('total_files', 1), 1) * 100
                ),
