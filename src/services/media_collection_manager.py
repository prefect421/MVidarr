...
class MediaCollectionManager:
    ...
    def __init__(self, config: Optional[CollectionProcessingConfig] = None):
        ...
        logger.info(f"📊 Media collection manager initialized with config: {self.config.to_dict()}")
    ...
    async def process_collection_metadata(
        ...
        if priority != ProcessingPriority.URGENT:
            ...
            logger.info(f"⏳ Queued metadata processing for collection {collection_id} (priority: {priority.value})")
    ...
    async def export_collection_metadata(
        ...
        try:
            ...
            logger.info(f"📤 Collection metadata exported: {collection_id} ({export_format})")
            return result
