...
class WebhookService:
            ...
            for endpoint in self.endpoints:
                if endpoint.url == url:
                    ...
                    if 'events' in updates:
                        endpoint.events = [WebhookEventType(event) for event in updates['events']]
    ...
    def trigger_event(self, event_type: WebhookEventType, data: Dict, metadata: Dict = None):
        ...
        try:
            ...
            logger.info(f"Triggered webhook event: {event_type.value} to {len(matching_endpoints)} endpoints")
    ...
    def _deliver_webhook(self, endpoint: WebhookEndpoint, event: WebhookEvent):
        ...
        def delivery_task():
            ...
            for attempt in range(endpoint.max_retries + 1):
                try:
                    ...
                    if response.status_code in [200, 201, 202, 204]:
                        logger.info(f"Webhook delivered successfully to {endpoint.url} (attempt {attempt + 1})")
                        return
                    else:
                        logger.warning(f"Webhook delivery failed to {endpoint.url}: HTTP {response.status_code}")
                        
                except requests.exceptions.RequestException as e:
                    logger.warning(f"Webhook delivery failed to {endpoint.url} (attempt {attempt + 1}): {e}")
            ...
            logger.error(f"Webhook delivery failed permanently to {endpoint.url} after {endpoint.max_retries + 1} attempts")
...
def trigger_artist_updated(artist_data: Dict, metadata: Dict = None):
    ...
    webhook_service.trigger_event(WebhookEventType.ARTIST_UPDATED, artist_data, metadata)

def trigger_artist_deleted(artist_data: Dict, metadata: Dict = None):
    """Trigger artist deleted event"""
    webhook_service.trigger_event(WebhookEventType.ARTIST_DELETED, artist_data, metadata)
...
def trigger_video_downloaded(video_data: Dict, metadata: Dict = None):
    ...
    webhook_service.trigger_event(WebhookEventType.VIDEO_DOWNLOADED, video_data, metadata)
...
def trigger_video_download_failed(video_data: Dict, error: str, metadata: Dict = None):
    ...
    data['error'] = error
    webhook_service.trigger_event(WebhookEventType.VIDEO_DOWNLOAD_FAILED, data, metadata)

def trigger_download_started(download_data: Dict, metadata: Dict = None):
    """Trigger download started event"""
    webhook_service.trigger_event(WebhookEventType.DOWNLOAD_STARTED, download_data, metadata)

def trigger_download_completed(download_data: Dict, metadata: Dict = None):
    """Trigger download completed event"""
    webhook_service.trigger_event(WebhookEventType.DOWNLOAD_COMPLETED, download_data, metadata)
...
def trigger_playlist_sync_started(playlist_data: Dict, metadata: Dict = None):
    ...
    webhook_service.trigger_event(WebhookEventType.PLAYLIST_SYNC_STARTED, playlist_data, metadata)

def trigger_playlist_sync_completed(playlist_data: Dict, results: Dict, metadata: Dict = None):
    """Trigger playlist sync completed event"""
    data = playlist_data.copy()
    data['results'] = results
    webhook_service.trigger_event(WebhookEventType.PLAYLIST_SYNC_COMPLETED, data, metadata)

def trigger_external_import_started(service: str, import_data: Dict, metadata: Dict = None):
    """Trigger external import started event"""
    data = import_data.copy()
    data['service'] = service
    webhook_service.trigger_event(WebhookEventType.EXTERNAL_IMPORT_STARTED, data, metadata)

def trigger_external_import_completed(service: str, import_data: Dict, results: Dict, metadata: Dict = None):
    """Trigger external import completed event"""
    data = import_data.copy()
    data['service'] = service
    data['results'] = results
    webhook_service.trigger_event(WebhookEventType.EXTERNAL_IMPORT_COMPLETED, data, metadata)

def trigger_system_health_changed(health_data: Dict, metadata: Dict = None):
    """Trigger system health changed event"""
    webhook_service.trigger_event(WebhookEventType.SYSTEM_HEALTH_CHANGED, health_data, metadata)
