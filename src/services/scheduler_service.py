...
class SchedulerService:
    ...
    def _setup_scheduled_jobs(self):
        ...
        try:
            ...
            logger.info(f"Setting up scheduled downloads for {schedule_days} at {schedule_time}")
            ...
            try:
                ...
                logger.error(f"Invalid schedule time format: {schedule_time}. Using default 02:00")
                schedule_time_obj = dt_time(2, 0)
                hour, minute = 2, 0
            
            # Schedule based on frequency
            if schedule_days == 'daily':
                schedule.every().day.at(f"{hour:02d}:{minute:02d}").do(self._run_scheduled_download)
                logger.info(f"Scheduled daily downloads at {hour:02d}:{minute:02d}")
            
            elif schedule_days == 'weekly':
                # Default to Sunday for weekly
                schedule.every().sunday.at(f"{hour:02d}:{minute:02d}").do(self._run_scheduled_download)
                logger.info(f"Scheduled weekly downloads on Sunday at {hour:02d}:{minute:02d}")
                ...
                if ',' in schedule_days:
                    selected_days = [day.strip().lower() for day in schedule_days.split(',')]
                    for day in selected_days:
                        if day in days_map:
                            days_map[day].at(f"{hour:02d}:{minute:02d}").do(self._run_scheduled_download)
                            logger.info(f"Scheduled downloads on {day.capitalize()} at {hour:02d}:{minute:02d}")
                        else:
                    ...
                    if day in days_map:
                        ...
                        logger.error(f"Invalid schedule days: {schedule_days}. Defaulting to daily")
                        schedule.every().day.at(f"{hour:02d}:{minute:02d}").do(self._run_scheduled_download)
        ...
        while self.running:
            try:
                ...
                if (self._last_check_time is None or 
                    (current_time - self._last_check_time).total_seconds() > 300):
    ...
    def _run_scheduled_download(self):
        ...
        try:
            ...
            if result.get('success'):
                ...
                logger.info(f"Scheduled download completed: {success_count} queued, {failed_count} failed, {total_wanted} total wanted videos")
                
                # Log summary
                if success_count > 0:
                    logger.info(f"Successfully queued {success_count} videos for download")
                if failed_count > 0:
