...
from src.database.models import Artist, Video, VideoStatus, Download, PlaylistMonitor
...
class YouTubePlaylistService:
    ...
    def create_playlist_monitor(self, playlist_url: str, name: str = None, auto_download: bool = True, 
                              quality: str = '720p', keywords: List[str] = None) -> Dict:
        ...
        try:
            ...
            with get_db() as session:
                ...
                existing_monitor = session.query(PlaylistMonitor).filter(
                    PlaylistMonitor.playlist_id == playlist_id
                ).first()
    ...
    def sync_playlist_videos(self, playlist_id: str, session: Session = None) -> Dict:
        ...
        try:
            # Get monitor
            monitor = session.query(PlaylistMonitor).filter(
                PlaylistMonitor.playlist_id == playlist_id
            ).first()
            ...
            for video_data in playlist_videos:
                try:
                    ...
                    if keywords and not self._matches_keywords(video_data['title'], keywords):
                        continue
                    ...
                    if video_id in existing_videos:
                        ...
                        artist = self._find_or_create_artist(video_data['channel_title'], session)
                        ...
                        video = Video(
                            ...
                            status=VideoStatus.WANTED if monitor.auto_download else VideoStatus.MONITORED,
                        ...
                        if monitor.auto_download:
                            try:
                                download_result = self.download_video(video_id, monitor.quality)
                                if download_result.get('success'):
                                    video.status = VideoStatus.DOWNLOADING
                                    results['downloaded_videos'] += 1
                            except Exception as e:
                                logger.error(f"Failed to download video {video_id}: {e}")
                                results['errors'].append(f"Download failed for {video_data['title']}: {str(e)}")
    ...
    def _find_or_create_artist(self, channel_title: str, session: Session) -> Artist:
        ...
        existing_artist = session.query(Artist).filter(
            Artist.name.ilike(f"%{channel_title}%")
        ).first()
    ...
    def download_video(self, video_id: str, quality: str = '720p') -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"MeTube download failed: {response.status_code} - {response.text}")
                return {
                    'success': False,
        ...
        try:
            with get_db() as session:
                ...
                for monitor in monitors:
                    # Get video count
                    video_count = session.query(Video).filter(
                        Video.playlist_id == monitor.playlist_id
                    ).count()
                    ...
                    playlist_data.append({
                        ...
                        'keywords': json.loads(monitor.keywords) if monitor.keywords else [],
    ...
    def delete_playlist_monitor(self, monitor_id: int, delete_videos: bool = False) -> Dict:
        ...
        try:
            with get_db() as session:
                ...
                if delete_videos:
                    videos_deleted = session.query(Video).filter(
                        Video.playlist_id == playlist_id
                    ).delete()
                    logger.info(f"Deleted {videos_deleted} videos from playlist {playlist_id}")
    ...
    def sync_all_playlists(self) -> Dict:
        ...
        try:
            with get_db() as session:
                ...
                for monitor in monitors:
                    try:
                        sync_result = self.sync_playlist_videos(monitor.playlist_id, session)
