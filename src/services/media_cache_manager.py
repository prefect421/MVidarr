...
class MediaCacheManager:
    ...
    def __init__(self, config: Optional[CacheConfiguration] = None):
        ...
        logger.info(f"🗄️ Media cache manager initialized with {self.config.max_memory_mb}MB limit")
    ...
    async def get(
        ...
        **kwargs
        ...
        try:
            ...
            logger.debug(f"✅ Cache hit: {cache_type.value}:{identifier} ({retrieval_time:.1f}ms)")
            return value
        ...
        **kwargs
        ...
        try:
            ...
            if success:
                ...
                logger.debug(f"💾 Cached: {cache_type.value}:{identifier} ({len(compressed)} bytes, TTL: {ttl}s)")
    ...
    async def invalidate_pattern(self, cache_type: CacheType, pattern: str = "*") -> int:
        ...
        try:
            ...
            if deleted_count > 0:
                ...
                logger.info(f"🧹 Invalidated {deleted_count} cache entries for pattern: {search_pattern}")
    ...
    async def optimize_cache_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.metrics.hit_ratio > 90:
                ...
                logger.info("📈 High cache hit ratio detected - optimizing for longer retention")
            elif self.metrics.hit_ratio < 50:
                # Low hit ratio - reduce TTLs to free memory
                logger.info("📉 Low cache hit ratio detected - optimizing for memory usage")
