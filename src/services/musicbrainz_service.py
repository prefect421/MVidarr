...
class MusicBrainzService:
    ...
    def _load_settings(self):
        ...
        try:
            ...
            self._enabled = settings.get("musicbrainz_enabled", "true").lower() == "true"
    ...
    def search_artist(self, artist_name: str) -> List[Dict]:
        ...
        for artist in data.get("artists", []):
            ...
            confidence = self._calculate_name_similarity(artist_name, artist.get("name", ""))
        ...
        if relations_data:
            for relation in relations_data:
                try:
                    ...
                    if rel_type in ["official homepage", "social network", "streaming music", "purchase for download"]:
    ...
    def get_artist_metadata_for_enrichment(self, artist_name: str, mbid: str = None) -> Optional[Dict]:
        ...
        if mbid:
            ...
            if best_match.get("confidence", 0) < 0.7:
                logger.warning(f"Low confidence MusicBrainz match for {artist_name}: {best_match.get('confidence', 0)}")
                return None
        ...
        metadata = {
            ...
            # Core metadata
            "genres": artist_data.get("genres", []) + artist_data.get("tags", [])[:5],  # Use tags as additional genres
        ...
        logger.info(f"Successfully enriched artist metadata from MusicBrainz: {metadata['name']}")
        return metadata
    ...
    def test_connection(self) -> bool:
        ...
        try:
            # Simple test query
            data = self._make_request("artist/5b11f4ce-a62d-471e-81fc-a69a8278c7da")  # The Beatles MBID
