...
class ArtistIdentificationService:
                ...
                if metadata['artist_name'] and metadata['title']:
                    ...
                    if title_similarity > 0.5:  # Minimum similarity threshold
                        candidates.append({
                            ...
                            'source': 'imvdb',
                            'confidence': title_similarity * 0.9,  # Slightly reduce for API uncertainty
                            'metadata': metadata,
    ...
    def _deduplicate_and_score(self, candidates: List[Dict], song_title: str) -> List[Dict]:
        ...
        for artist_key, group in artist_groups.items():
            ...
            if len(group) > 1:
                ...
                best_candidate['confidence'] = min(1.0, best_candidate['confidence'] + source_bonus)
                
                # Update match reason to include multiple sources
                sources = list(set(c['source'] for c in group))
                best_candidate['match_reason'] += f" (confirmed by {', '.join(sources)})"
            
            # Additional scoring based on artist name quality
            name_quality_score = self._assess_artist_name_quality(best_candidate['artist_name'])
    ...
    def _assess_artist_name_quality(self, name: str) -> float:
        ...
        special_char_ratio = sum(not (c.isalnum() or c.isspace()) for c in name) / len(name)
    ...
    def get_identification_summary(self, session, limit: int = 20) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'identification_rate': len(identifiable_videos) / len(videos) if videos else 0,
