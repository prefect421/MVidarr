...
class VideoDiscoveryService:
    ...
    def discover_videos_for_artist(self, artist_id: int, limit: int = 10) -> Dict:
        ...
        try:
            with get_db() as session:
                ...
                existing_videos = session.query(Video).filter_by(artist_id=artist_id).all()
                existing_urls = {video.url for video in existing_videos if video.url}
                logger.info(f"Found {len(existing_videos)} existing videos for {artist.name}, {len(existing_urls)} with URLs")
                ...
                logger.info(f"IMVDb returned {len(imvdb_results)} videos for {artist.name}")
                
                for video_data in imvdb_results:
                    if video_data.get('url'):
                        if video_data['url'] not in existing_urls:
                            # Check if already found via YouTube
                            if not any(v['url'] == video_data['url'] for v in discovered_videos):
                                discovered_videos.append({
                                    'source': 'imvdb',
                                    **video_data
                                })
                                ...
                                logger.info(f"Video already in discovered list: {video_data['title']}")
                        else:
                            logger.info(f"Video URL already exists: {video_data['title']} - {video_data['url']}")
                    else:
                        logger.info(f"Video has no URL: {video_data.get('title', 'Unknown')}")
                    time.sleep(self.rate_limit_delay)
                ...
                logger.info(f"Discovery complete for {artist.name}: found {len(discovered_videos)} videos, stored {stored_count}")
    ...
    def discover_videos_for_all_artists(self, limit_per_artist: int = 5) -> Dict:
        ...
        try:
            with get_db() as session:
                ...
                for artist in artists:
                    try:
                        ...
                        if not self._should_discover_for_artist(artist):
                            logger.debug(f"Skipping discovery for {artist.name} - too recent")
                            continue
                        
                        result = self.discover_videos_for_artist(artist.id, limit_per_artist)
                        results.append({
                            'artist_id': artist.id,
                            'artist_name': artist.name,
                            'result': result
                ...
                logger.info(f"Bulk discovery complete: {total_discovered} videos discovered, {total_stored} stored")
    ...
    def _search_imvdb_for_artist(self, artist: Artist, limit: int) -> List[Dict]:
        ...
        try:
            ...
            for video in results['videos']:
                ...
                song_title = str(video.get('song_title', 'Unknown')) if video.get('song_title') is not None else 'Unknown'
                video_data = {
                    ...
                    'thumbnail_url': video.get('image', {}).get('l') if video.get('image') else None
                }
                ...
                if youtube_url:
                    ...
                    logger.debug(f"No YouTube URL for {video.get('song_title')}, using IMVDb URL")
                
                video_list.append(video_data)
                logger.info(f"Added video: {video_data['title']} - URL: {video_data['url']}")
    ...
    def _store_discovered_video(self, session, artist_id: int, video_data: Dict):
        ...
        try:
            ...
            video_title = str(video_data.get('title', '')) if video_data.get('title') is not None else ''
            video = Video(
                artist_id=artist_id,
                title=video_title,
                url=video_data.get('url'),
            ...
            if video.thumbnail_url:
                try:
                    ...
                    logger.error(f"Error downloading thumbnail for video {video.title}: {e}")
    ...
    def get_discovery_stats(self) -> Dict:
        ...
        try:
            with get_db() as session:
                ...
                wanted_videos = session.query(Video).filter_by(status=VideoStatus.WANTED).count()
                downloaded_videos = session.query(Video).filter_by(status=VideoStatus.DOWNLOADED).count()
                
                # Count artists
                total_artists = session.query(Artist).count()
                monitored_artists = session.query(Artist).filter_by(monitored=True).count()
                
                # Recent discovery stats (last 7 days)
                week_ago = datetime.utcnow() - timedelta(days=7)
                recent_discoveries = session.query(Video).filter(
                    Video.discovered_date >= week_ago
                ).count()
