...
from src.database.models import Artist, Video, VideoStatus, Setting
...
class PlexService:
    ...
    def _load_settings(self):
        ...
        try:
            ...
            self.server_url = SettingsService.get('plex_server_url', 'http://localhost:32400')
    ...
    def search_library(self, query: str, library_key: str = None) -> List[Dict]:
        ...
        try:
            ...
            response = self._make_request(f'/library/sections/{library_key}/search', params)
            root = ET.fromstring(response.content)
                ...
                for plex_artist in plex_artists:
                    try:
                        ...
                        if existing_artist:
                            ...
                            existing_artist.imvdb_metadata = existing_artist.imvdb_metadata or {}
    ...
    def sync_mvidarr_to_plex(self) -> Dict:
        ...
        try:
            ...
            with get_db() as session:
                mvidarr_artists = session.query(Artist).filter(
                ...
                for artist in mvidarr_artists:
                    try:
                        ...
                        if search_results:
                            ...
                            logger.info(f"Matched MVidarr artist '{artist.name}' to Plex: {best_match['title']}")
                        else:
                            results['unmatched_artists'] += 1
                            logger.debug(f"No Plex match found for artist: {artist.name}")
                        
                    except Exception as e:
                        error_msg = f"Failed to search for artist {artist.name}: {str(e)}"
                        logger.error(error_msg)
                        results['errors'].append(error_msg)
    ...
    def get_artist_listening_stats(self, artist_name: str) -> Dict:
        ...
        try:
            ...
            if not search_results:
                return {
                    'found': False,
                    'error': 'Artist not found in Plex library'
                }
    ...
    def get_recently_played(self, limit: int = 50) -> List[Dict]:
        ...
        try:
            ...
            response = self._make_request(f'/library/sections/{music_lib["key"]}/recentlyViewed', params)
            root = ET.fromstring(response.content)
        ...
        try:
            ...
            for artist in artists[:10]:  # Sample first 10 artists
                try:
                    ...
                    logger.warning(f"Failed to get details for artist {artist['title']}: {e}")
            ...
            if len(artists) > 0:
                ...
                library_stats['estimated_album_count'] = int(avg_albums_per_artist * len(artists))
                library_stats['estimated_track_count'] = int(avg_tracks_per_album * library_stats['estimated_album_count'])
            else:
