...
class VideoIndexingService:
    ...
    def fetch_imvdb_metadata(self, artist_name: str, title: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(f"Failed to fetch IMVDb metadata for {artist_name} - {title}: {e}")
            return None
    
    def create_video_record(self, artist: Artist, file_metadata: Dict, imvdb_metadata: Dict = None, session=None) -> Video:
        """
            ...
            if imvdb_metadata.get('thumbnail_url'):
                try:
                    ...
                    if thumbnail_path:
                        ...
                        logger.info(f"Downloaded thumbnail for: {artist.name} - {video.title}")
                except Exception as e:
                    logger.warning(f"Failed to download thumbnail for {artist.name} - {video.title}: {e}")
    ...
    def create_download_record(self, artist: Artist, video: Video, file_metadata: Dict, session=None) -> Download:
        """
    ...
    def index_single_file(self, file_path: Path, fetch_metadata: bool = True) -> Dict:
        ...
        try:
            ...
            result['video_title'] = file_metadata['extracted_title'] or file_metadata['filename']
            
            with get_db() as session:
                # Find or create artist
                artist = self.find_or_create_artist(file_metadata['extracted_artist'], session)
                ...
                if existing_video:
                    ...
                    if imvdb_metadata and not video.imvdb_id:
                        ...
                        if imvdb_metadata.get('thumbnail_url') and not video.thumbnail_path:
                            try:
                                thumbnail_path = thumbnail_service.download_video_thumbnail(
                                    artist.name, 
                                    video.title, 
                                ...
                                if thumbnail_path:
                                    ...
                                    logger.info(f"Downloaded thumbnail for existing video: {artist.name} - {video.title}")
                            except Exception as e:
                                logger.warning(f"Failed to download thumbnail for {artist.name} - {video.title}: {e}")
                else:
                    video = self.create_video_record(artist, file_metadata, imvdb_metadata, session)
                    result['video_created'] = True
                    if imvdb_metadata and imvdb_metadata.get('thumbnail_url'):
                        result['thumbnail_downloaded'] = True
                
                # Create download record
                download = self.create_download_record(artist, video, file_metadata, session)
    ...
    def index_all_videos(self, directory: Path = None, fetch_metadata: bool = True, max_files: int = None) -> Dict:
        """
        ...
        logger.info(f"Indexing complete: {successful} successful, {failed} failed, {already_indexed} already indexed")
        logger.info(f"Created: {artists_created} artists, {videos_created} videos, {downloads_created} downloads")
    ...
    def get_indexing_stats(self) -> Dict:
        ...
        try:
            with get_db() as session:
                ...
                videos_with_imvdb = session.query(Video).filter(
                ...
                videos_with_files = session.query(Download).filter(
                ...
                return {
                    ...
                    'imvdb_coverage': round((videos_with_imvdb / total_videos * 100), 2) if total_videos > 0 else 0
                }
