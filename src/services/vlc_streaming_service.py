...
class VLCStreamingService:
        ...
        try:
            ...
            if video_id in self.active_streams:
                ...
                if self._is_stream_active(stream_info['port']):
                    logger.info(f"Stream already active for video {video_id} on port {stream_info['port']}")
                    return {
                        'success': True,
            ...
            if self._wait_for_stream_ready(port):
                logger.info(f"VLC HTTP interface started for video {video_id} on port {port}")
                return {
                    'success': True,
    ...
    def _is_stream_active(self, port):
        ...
        try:
            response = requests.get(f'http://localhost:{port}/stream.mp4', 
