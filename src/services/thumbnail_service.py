...
class ThumbnailService:
    ...
    def generate_filename(self, url: str, prefix: str = '') -> str:
        ...
        filename = f"{prefix}{url_hash}{extension}" if prefix else f"{url_hash}{extension}"
        return filename
    
    def download_thumbnail(self, url: str, filename: str = None, subdirectory: str = 'videos') -> Optional[str]:
        """
    ...
    def _validate_and_process_image(self, image_path: Path, max_size: tuple = (800, 600)) -> bool:
        """
    ...
    def download_video_thumbnail(self, artist_name: str, video_title: str, url: str) -> Optional[str]:
        """
    ...
    def upload_manual_thumbnail(self, file_data: bytes, filename: str, entity_type: str, entity_id: int, entity_name: str) -> Optional[Dict]:
        """
        ...
        try:
            ...
            for size_name, dimensions in sizes_config.items():
                ...
                logger.debug(f"Generated {size_name} thumbnail: {file_path} ({processed_image.size})")
            ...
            logger.info(f"Successfully uploaded and processed manual thumbnail for {entity_type} {entity_id}: {base_filename}")
            return result
    ...
    def crop_thumbnail(self, image_path: str, crop_box: Tuple[int, int, int, int], entity_type: str, entity_id: int, entity_name: str) -> Optional[Dict]:
        """
        ...
        try:
            ...
            with Image.open(image_path) as image:
                ...
                if left < 0 or top < 0 or right > img_width or bottom > img_height or left >= right or top >= bottom:
                    logger.error(f"Invalid crop box: {crop_box} for image size {image.size}")
                    return None
                ...
                if upload_result:
                    ...
                    logger.info(f"Successfully cropped thumbnail for {entity_type} {entity_id}")
    ...
    def delete_thumbnail_files(self, thumbnail_path: str, metadata: Dict = None) -> bool:
        """
                ...
                if category_dir.exists():
                    for file_path in category_dir.rglob('*'):
                        if file_path.is_file():
                            ...
                            # Size counting
                            for size in ['small', 'medium', 'large', 'original']:
                                if size in file_path.name or file_path.parent.name == size:
