...
class EnhancedArtistDiscoveryService:
    ...
    def discover_artists_multi_source(
        ...
        if cache_key in self.discovery_cache:
            ...
            if datetime.utcnow() - cache_entry['timestamp'] < timedelta(hours=self.cache_duration_hours):
        ...
        try:
            ...
            logger.info(f"IMVDb returned {len(imvdb_results)} artists for: {search_query}")
        ...
        try:
            ...
            logger.info(f"Spotify returned {len(spotify_results)} artists for: {search_query}")
        except Exception as e:
        ...
        try:
            ...
            logger.info(f"Last.fm returned {len(lastfm_results)} artists for: {search_query}")
        except Exception as e:
        ...
        try:
            ...
            logger.info(f"Wikipedia returned {len(wikipedia_results)} artists for: {search_query}")
        except Exception as e:
        ...
        filtered_artists = [
            artist for artist in ranked_artists 
        ...
        logger.info(f"Multi-source discovery completed: {len(filtered_artists)} high-quality results")
        ...
        try:
            with get_db() as db:
                ...
                if enrichment_data:
                    ...
                    applied_fields = self._apply_enrichment_to_artist(artist, merged_metadata)
                    
                    db.commit()
                    
                    logger.info(f"Artist {artist.name} enriched with fields: {applied_fields}")
                    return True
                else:
                    logger.warning(f"No enrichment data found for artist: {artist.name}")
                    return False
    ...
    def detect_duplicate_artists(self, limit: int = 100) -> List[DuplicateCandidate]:
        ...
        try:
            with get_db() as db:
                ...
                artists = db.query(Artist).order_by(desc(Artist.id)).limit(limit * 2).all()
    ...
    def get_artist_recommendations(self, artist_id: int, limit: int = 20) -> List[ArtistMetadata]:
        """
        ...
        try:
            with get_db() as db:
                ...
                if artist.genres:
                    genre_recs = self._get_genre_based_recommendations(artist.genres.split(','), limit // 2)
                    recommendations.extend(genre_recs)

                # 2. Similar artist recommendations from Last.fm
                try:
                    similar_recs = self._get_similar_artist_recommendations(artist.name, limit // 2)
                ...
                try:
                    collab_recs = self._get_collaborative_recommendations(artist_id, limit // 4)
                ...
                logger.info(f"Generated {len(ranked_recommendations)} recommendations for {artist.name}")
                return ranked_recommendations

        except Exception as e:
            logger.error(f"Error generating recommendations for artist {artist_id}: {e}")
            return []
    ...
    def _discover_from_imvdb(self, search_query: str) -> List[ArtistMetadata]:
        ...
        try:
            results = imvdb_service.search_artists(search_query, self.max_results_per_source)
    ...
    def _discover_from_spotify(self, search_query: str) -> List[ArtistMetadata]:
        ...
        try:
            ...
            artists = self.spotify_service.search_artist(search_query, limit=self.max_results_per_source)
            ...
            for artist in artists.get('artists', {}).get('items', []):
                ...
                metadata = ArtistMetadata(
                    ...
                    quality_score=MetadataQuality.GOOD if genres else MetadataQuality.FAIR
                )
                artist_metadata.append(metadata)
    ...
    def _discover_from_lastfm(self, search_query: str) -> List[ArtistMetadata]:
        ...
        try:
            ...
            results = {'results': {'artistmatches': {'artist': [artist_info] if artist_info else []}}}
            
            artist_metadata = []
            for artist in results.get('results', {}).get('artistmatches', {}).get('artist', []):
                metadata = ArtistMetadata(
                    name=artist.get('name', ''),
                    ...
                    image_url=artist.get('image', [{}])[-1].get('#text', '') if artist.get('image') else '',
    ...
    def _discover_from_wikipedia(self, search_query: str) -> List[ArtistMetadata]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                music_keywords = ['singer', 'musician', 'band', 'artist', 'album', 'song', 'music']
                ...
                if is_music_related:
                    metadata = ArtistMetadata(
                        ...
                        external_ids={'wikipedia_url': data.get('content_urls', {}).get('desktop', {}).get('page', '')},
                        quality_score=MetadataQuality.FAIR
                    )
                    return [metadata]
    ...
    def _merge_duplicate_discoveries(self, discoveries: List[ArtistMetadata]) -> List[ArtistMetadata]:
    ...
    def _merge_artist_metadata(self, discoveries: List[ArtistMetadata]) -> ArtistMetadata:
        """Merge multiple discoveries of the same artist"""
        if not discoveries:
            return None
        ...
        merged = ArtistMetadata(
            ...
            confidence=min(1.0, sum(d.confidence for d in discoveries) / len(discoveries) * 1.2),
            genres=[],
        ...
        for discovery in discoveries:
            if discovery.biography and len(discovery.biography) > len(merged.biography or ''):
    ...
    def _rank_and_score_discoveries(self, discoveries: List[ArtistMetadata]) -> List[ArtistMetadata]:
    ...
    def _calculate_artist_similarity(self, artist1: Artist, artist2: Artist) -> DuplicateCandidate:
        ...
        if artist1.country and artist2.country:
            country_sim = 1.0 if artist1.country.lower() == artist2.country.lower() else 0.0
        ...
        try:
            ...
            if artist_info:
                return ArtistMetadata(
                    ...
                    similar_artists=[a.get('name', '') for a in artist_info.get('similar', {}).get('artist', [])],
                    quality_score=MetadataQuality.GOOD if artist_info.get('bio') else MetadataQuality.FAIR
                )
        except Exception as e:
    ...
    def _enrich_from_wikipedia(self, artist_name: str) -> Optional[ArtistMetadata]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                music_keywords = ['singer', 'musician', 'band', 'artist', 'album', 'song']
    ...
    def _merge_enrichment_data(self, enrichment_data: List[ArtistMetadata]) -> ArtistMetadata:
        ...
        merged = ArtistMetadata(
            ...
            confidence=min(1.0, sum(d.confidence for d in enrichment_data) / len(enrichment_data)),
            genres=[],
    ...
    def _apply_enrichment_to_artist(self, artist: Artist, metadata: ArtistMetadata) -> List[str]:
    ...
    def _extract_country_from_text(self, text: str) -> Optional[str]:
        ...
        countries = [
            ...
            'France', 'Italy', 'Spain', 'Sweden', 'Norway', 'Denmark', 'Japan',
            'South Korea', 'Brazil', 'Mexico', 'Argentina', 'India', 'Russia'
        ]
    ...
    def _get_genre_based_recommendations(self, genres: List[str], limit: int) -> List[ArtistMetadata]:
        ...
        try:
            for genre in genres[:3]:  # Limit to first 3 genres
                spotify_recs = self._get_spotify_genre_recommendations(genre.strip(), limit // len(genres))
    ...
    def _get_spotify_genre_recommendations(self, genre: str, limit: int) -> List[ArtistMetadata]:
    ...
    def _get_similar_artist_recommendations(self, artist_name: str, limit: int) -> List[ArtistMetadata]:
        ...
        try:
            ...
            if not hasattr(self.lastfm_service, 'api_key') or not self.lastfm_service.api_key:
            ...
            response = requests.get(self.lastfm_service.base_url, params=params, timeout=10)
            ...
            if response.status_code == 200:
                ...
                for similar_artist in data.get('similarartists', {}).get('artist', []):
                    metadata = ArtistMetadata(
                        ...
                        image_url=similar_artist.get('image', [{}])[-1].get('#text', '') if similar_artist.get('image') else '',
                        quality_score=MetadataQuality.FAIR
                    )
                ...
                logger.warning(f"Last.fm similar artists request failed: {response.status_code}")
                return []
    ...
    def _get_collaborative_recommendations(self, artist_id: int, limit: int) -> List[ArtistMetadata]:
        ...
        try:
            with get_db() as db:
                ...
                similar_artists = db.query(Artist).filter(
                    and_(
                        Artist.id != artist_id,
                        Artist.genres.ilike(f'%{artist.genres.split(",")[0].strip()}%')
                    )
                ).limit(limit).all()
                ...
                for similar_artist in similar_artists:
                    metadata = ArtistMetadata(
                        ...
                        confidence=0.5,
                        genres=similar_artist.genres.split(',') if similar_artist.genres else [],
                        quality_score=MetadataQuality.FAIR
                    )
                    recommendations.append(metadata)
    ...
    def _deduplicate_recommendations(self, recommendations: List[ArtistMetadata]) -> List[ArtistMetadata]:
