...
from src.database.models import Playlist, PlaylistEntry, Video, Artist, PlaylistType
...
class DynamicPlaylistService:
    ...
    def create_dynamic_playlist(
        ...
        with get_db() as session:
            ...
            self.logger.info(f"Created dynamic playlist '{name}' (ID: {playlist.id}) for user {user_id}")
            return playlist

    @monitor_performance("dynamic_playlist.update")
    def update_dynamic_playlist(self, playlist_id: int, session: Optional[Session] = None) -> bool:
        ...
        try:
            ...
            if not playlist.filter_criteria:
                self.logger.warning(f"Dynamic playlist {playlist_id} has no filter criteria")
                return False
            ...
            matching_videos = self.execute_filter_criteria(playlist.filter_criteria, session)
            ...
            if videos_to_remove:
                ...
                changes_made = True
                self.logger.debug(f"Removed {len(videos_to_remove)} videos from playlist {playlist_id}")
            ...
            if videos_to_add:
                ...
                changes_made = True
                self.logger.debug(f"Added {len(videos_to_add)} videos to playlist {playlist_id}")
            ...
            if changes_made:
                self.logger.info(f"Updated dynamic playlist {playlist_id}: +{len(videos_to_add)}, -{len(videos_to_remove)} videos")
            else:
                self.logger.debug(f"No changes needed for dynamic playlist {playlist_id}")
    ...
    def execute_filter_criteria(self, filter_criteria: Dict[str, Any], session: Session) -> List[Video]:
            ...
            for keyword in keywords:
                keyword_conditions.append(
                    or_(
                        ...
                        Video.description.ilike(f"%{keyword}%") if Video.description else False
                    )
                )
    ...
    def update_all_dynamic_playlists(self, max_age_hours: int = 24) -> Dict[str, Any]:
        ...
        with get_db() as session:
            ...
            playlists_to_update = session.query(Playlist).filter(
                and_(
            ...
            for playlist in playlists_to_update:
                try:
                    changes_made = self.update_dynamic_playlist(playlist.id, session=session)
                    if changes_made:
                        total_changes += 1
                    updated_count += 1
                except Exception as e:
    ...
    def preview_filter_criteria(self, filter_criteria: Dict[str, Any], limit: int = 50) -> Dict[str, Any]:
