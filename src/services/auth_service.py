...
class AuthService:
    ...
    @staticmethod
    def create_user(username: str, email: str, password: str, role: UserRole = UserRole.USER) -> Tuple[bool, str, Optional[User]]:
        """
        Create a new user
    ...
    @staticmethod
    def authenticate_user(username: str, password: str, ip_address: str = None, user_agent: str = None, 
                         two_factor_token: str = None) -> Tuple[bool, str, Optional[dict], Optional[dict], bool]:
        """
        ...
        try:
            with get_db() as session:
                ...
                if user.is_locked():
                    ...
                    return False, f"Account is locked until {user.locked_until}", None, None, False
                ...
                if user.two_factor_enabled:
                    if not two_factor_token:
                        ...
                        return True, "Two-factor authentication required", user_data, None, True
                    
                    # Verify 2FA token
                    from src.services.two_factor_service import TwoFactorService
                    totp_valid, totp_message = TwoFactorService.verify_two_factor_login(user.id, two_factor_token)
                    
                    if not totp_valid:
                        logger.warning(f"Failed 2FA attempt for user: {username} from {ip_address}")
                ...
                logger.info(f"User authenticated: {username} from {ip_address} (2FA: {user.two_factor_enabled})")
        ...
        try:
            with get_db() as session:
                ...
                user_data = {
                    ...
                    'can_modify_content': user.role.value in ['ADMIN', 'MANAGER', 'USER'],
    ...
    @staticmethod
    def create_user_session(user: User, ip_address: str = None, user_agent: str = None) -> str:
        """
    ...
    @staticmethod
    def revoke_all_user_sessions_except_current(user_id: int, current_token: str) -> bool:
        """
        ...
        try:
            with get_db() as session:
                user_sessions = session.query(UserSession).filter(
                    ...
                    UserSession.session_token != current_token.replace('Bearer ', '')
                ).all()
                ...
                session.commit()
                logger.info(f"Revoked {len(user_sessions)} sessions for user ID: {user_id}")
                return True
    ...
    @staticmethod
    def update_user_role(user_id: int, new_role: UserRole, admin_user_id: int) -> Tuple[bool, str]:
        """
            ...
            with get_db() as session:
                ...
                if admin_count == 0:
                    ...
                    if success:
                        ...
                        return True, "Default admin user created with username 'admin' and password 'MVidarr@dmin123'. Please change the password immediately."
                    else:
