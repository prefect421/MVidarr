...
class IMVDbService:
        ...
        if year_min or year_max:
            ...
            if video_year:
                try:
                    ...
                    if year_min or year_max:  # If year filters specified but video has no valid year
                        return False
        ...
        if directors_filter:
            ...
            for director_filter in directors_filter:
                ...
                for video_director in video_director_names:
                    if director_filter in video_director or video_director in director_filter:
                        found_director = True
                        break
                if found_director:
                    break
    ...
    def _sort_videos(self, videos: List[Dict], sort_by: str, sort_order: str) -> List[Dict]:
        ...
        if sort_by == "year":
            ...
            return sorted(videos, key=lambda v: v.get("song_title", "").lower(), reverse=reverse)
        elif sort_by == "artist":
            ...
            return sorted(videos, key=lambda v: v.get("relevance_score", 0), reverse=reverse)
        else:
    ...
    def search_videos_by_year_range(self, year_min: int, year_max: int, limit: int = 50) -> List[Dict]:
        """
    ...
    def analyze_video_quality(self, video_data: Dict) -> Dict:
        ...
        if isinstance(video_data.get("artist"), dict) and video_data["artist"].get("id"):
            source_score += 10
    ...
    def get_user_preferences(self, user_id: Optional[int] = None) -> Dict:
        ...
        default_preferences = {
            ...
            "preferred_genres": ["pop", "rock", "hip-hop", "electronic", "indie", "alternative"],
    ...
    def rank_videos_by_preferences(self, videos: List[Dict], user_id: Optional[int] = None) -> List[Dict]:
        """
        ...
        for video in videos:
            ...
            if year:
                try:
                    ...
                    if year_prefs.get("min", 0) <= year_int <= year_prefs.get("max", 9999):
            ...
            preferred_genres = [g.lower() for g in preferences.get("preferred_genres", [])]
            ...
            enhanced_video.update({
                ...
                "meets_quality_threshold": quality_analysis["overall_score"] >= min_quality
            })
    ...
    def get_quality_statistics(self, videos: List[Dict]) -> Dict:
        ...
        for video in videos:
            ...
            if year:
                try:
                    ...
                    year_distribution[f"{decade}s"] = year_distribution.get(f"{decade}s", 0) + 1
                except (ValueError, TypeError):
                    pass
        ...
        return {
            ...
            "quality_stats": {
                ...
                "medium_quality_count": len([s for s in quality_scores if 40 <= s < 70]),
