...
class ThumbnailOptimizationService:
    ...
    def convert_to_webp(self, input_path: Path, output_path: Path, quality: int = None) -> bool:
        """
        ...
        try:
            ...
            with Image.open(input_path) as img:
                ...
                logger.info(f"Converted {input_path.name} to WebP: {original_size} -> {webp_size} bytes ({space_saved} saved)")
                return True
    ...
    def optimize_existing_thumbnails(self, dry_run: bool = False) -> Dict[str, Any]:
                ...
                for img_file in current_dir.iterdir():
                    ...
                    if not dry_run:
                        ...
                        if self.convert_to_webp(img_file, webp_path):
                            ...
                            file_info['space_saved'] = file_info['original_size'] - file_info['webp_size']
                            ...
                            try:
                                ...
                                logger.warning(f"Could not remove original file {img_file}: {e}")
                        else:
                            results['errors'] += 1
                    else:
                        # Estimate space savings for dry run
                        estimated_webp_size = int(file_info['original_size'] * 0.7)  # Estimate 30% reduction
                        file_info['webp_size'] = estimated_webp_size
                        file_info['space_saved'] = file_info['original_size'] - estimated_webp_size
                        file_info['success'] = True
                        results['space_saved'] += file_info['space_saved']
        ...
        logger.info(f"Thumbnail optimization {'analysis' if dry_run else 'completed'}: "
                   f"{results['analyzed']} analyzed, {results['converted']} converted, "
                   f"{results['space_saved']} bytes saved")
        
        return results
    
    def create_optimized_sizes(self, source_path: Path, base_name: str, 
                              subdirectory: str = 'videos') -> Dict[str, str]:
        """
    ...
    def cleanup_duplicate_thumbnails(self) -> Dict[str, Any]:
            ...
            for img_file in thumbnail_dir.rglob('*'):
                ...
                try:
                    ...
                    if file_hash in file_hashes:
                        ...
                        if img_file.suffix.lower() == '.webp' and original_file.suffix.lower() != '.webp':
                        ...
                        elif img_file.suffix.lower() != '.webp' and original_file.suffix.lower() == '.webp':
        ...
        logger.info(f"Duplicate cleanup completed: {results['duplicates_found']} duplicates removed, "
                   f"{results['space_saved']} bytes saved")
    ...
    def analyze_storage_usage(self) -> Dict[str, Any]:
        ...
        for subdirectory in ['artists', 'videos']:
            ...
            for img_file in thumbnail_dir.rglob('*'):
                ...
                if file_format != '.webp':
                    analysis['optimization_potential'] += int(file_size * 0.3)  # Estimate 30% savings
    ...
    def get_optimization_recommendations(self) -> List[Dict[str, Any]]:
        ...
        non_webp_files = sum(stats['files'] for format_name, stats in analysis['by_format'].items() 
                           if format_name != '.webp')
        non_webp_size = sum(stats['size'] for format_name, stats in analysis['by_format'].items() 
                          if format_name != '.webp')
