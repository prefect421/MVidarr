...
class LastFmService:
    ...
    def get_user_top_artists(self, username: str = None, period: str = 'overall', 
    ...
    def get_recent_tracks(self, username: str = None, limit: int = 50, 
                         page: int = 1, from_timestamp: int = None, 
                         to_timestamp: int = None) -> Dict:
        ...
        for track_data in recent_tracks.get('track', []):
            ...
            track_info = {
                'name': track_data.get('name'),
                'artist': track_data.get('artist', {}).get('#text') or track_data.get('artist'),
                'album': track_data.get('album', {}).get('#text') or track_data.get('album'),
                'mbid': track_data.get('mbid'),
                'url': track_data.get('url'),
                'image': track_data.get('image', []),
    ...
    def get_artist_info(self, artist_name: str, username: str = None) -> Dict:
        ...
        return {
            ...
            'tags': [tag.get('name') for tag in artist_info.get('tags', {}).get('tag', [])]
        }
    
    def get_loved_tracks(self, username: str = None, limit: int = 50, page: int = 1) -> Dict:
    ...
    def import_top_artists(self, username: str = None, period: str = 'overall',
                          limit: int = 50, min_playcount: int = 1) -> Dict:
                        ...
                        if existing_artist:
                            ...
                            try:
                                ...
                                if imvdb_results and imvdb_results.get('results'):
                                    ...
                                    videos = imvdb_service.get_artist_videos(artist_match['id'])
                                    if videos:
                                        for video_data in videos[:10]:  # Limit to first 10
                                            ...
                                            if not existing_video:
                                                new_video = Video(
                                                    ...
                                                    thumbnail_url=video_data.get('image', {}).get('o'),
                                ...
                                logger.warning(f"Failed to get IMVDB data for {artist_name}: {e}")
                ...
                for artist_name, tracks in artist_tracks.items():
                    try:
                        ...
                        try:
                            ...
                            if artist.imvdb_id:
                                videos = imvdb_service.get_artist_videos(int(artist.imvdb_id))
                                if videos:
                            ...
                            logger.warning(f"Failed to get videos for {artist_name}: {e}")
    ...
    def get_listening_stats(self, username: str = None, days: int = 30) -> Dict:
        ...
        try:
            ...
            daily_pattern = [{'date': k, 'count': v} for k, v in sorted(daily_counts.items())]
