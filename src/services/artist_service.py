...
class ArtistService:
    ...
    def add_tracked_artist(self, user_id: int, artist_name: str, search_terms: str = None, 
        ...
        try:
            ...
            """
            existing = self.db.execute_query(existing_query, (user_id, artist_name), fetch=True)
            ...
            """
            artist_data = self.db.execute_query(artist_query, (user_id, artist_name), fetch=True)
    ...
    def remove_tracked_artist(self, user_id: int, artist_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            """
            artist_data = self.db.execute_query(check_query, (artist_id, user_id), fetch=True)
            ...
            if rows_affected > 0:
                logger.info(f"Artist '{artist_name}' (ID: {artist_id}) removed for user {user_id}")
                return {
                    'success': True,
    ...
    def update_artist_settings(self, user_id: int, artist_id: int, settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            allowed_fields = ['auto_download', 'quality_preference', 'format_preference', 'is_active', 'search_terms']
    ...
    def check_for_new_videos(self, user_id: int, artist_id: int = None) -> Dict[str, Any]:
            ...
            for artist in artists:
                try:
                    ...
                    if search_result['success']:
                        ...
                        self.db.execute_query(
                            update_query, 
                            (len(search_result['results']), artist['id'])
                        )
                        
                        checked_count += 1
                    
                except Exception as e:
                    logger.warning(f"Error checking artist {artist['artist_name']}: {e}")
                    continue
            
            logger.info(f"Checked {checked_count} artists for user {user_id}, found {len(new_videos)} new videos")
