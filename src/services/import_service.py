...
from src.database.models import (
    Artist, Video, Playlist, PlaylistEntry, Setting, VideoBlacklist,
    VideoStatus, User
)
from src.database.import_export_models import (
    ImportOperation, ImportOptions, ImportMode, ValidationLevel, ProcessingStatus,
    ValidationError, ValidationResult, ProcessingProgress, ExportData,
    ExportedArtist, ExportedVideo, ExportedPlaylist, ExportedSetting,
    ExportFormat
)
...
class ImportService:
        ...
        try:
            ...
            with get_db() as db:
                operation = ImportOperation(
                    ...
                    source_size_bytes=source_file_path.stat().st_size if source_file_path.exists() else 0,
            ...
            # Perform the import
            self._perform_import(operation_id, source_file_path, import_options, progress_callback)
    ...
    def validate_import_file(
        ...
        try:
            ...
            # Validate the data
            validation_result = self._validate_import_data(import_data, validation_level)
    ...
    def get_import_status(self, operation_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error getting import status for operation {operation_id}: {e}")
            return None
    ...
    def cancel_import(self, operation_id: int, user_id: int) -> bool:
        ...
        try:
            with get_db() as db:
                operation = db.query(ImportOperation).filter(
                    and_(
                        ImportOperation.id == operation_id,
                        ImportOperation.user_id == user_id,
                        ImportOperation.status.in_([ProcessingStatus.PENDING, ProcessingStatus.RUNNING])
                    )
                ).first()
    ...
    def _perform_import(
            ...
            try:
                with get_db() as db:
                    ...
                    if operation:
                        ...
                        operation.successful_records = progress.records_processed - progress.errors_count
                ...
                logger.error(f"Error updating progress for import operation {operation_id}: {e}")
        ...
        try:
            ...
            validation_result = self._validate_import_data(import_data, import_options.validation_level)
            ...
            if not validation_result.is_valid and import_options.validation_level == ValidationLevel.STRICT:
                raise ValueError(f"Validation failed with {len(validation_result.errors)} errors")
            ...
            import_results = self._import_data(import_data, import_options, progress, update_progress)
        ...
        try:
            ...
            if file_extension == '.gz':
                ...
                if inner_extension == 'json':
                    ...
                    raise ValueError(f"Unsupported compressed file format: {inner_extension}")
    ...
    def _validate_import_data(self, import_data: ExportData, validation_level: ValidationLevel) -> ValidationResult:
        ...
        try:
            ...
            for i, artist in enumerate(import_data.artists):
                total_records += 1
                artist_errors, artist_warnings = self._validate_artist(artist, f"artist_{i}")
            ...
            for i, playlist in enumerate(import_data.playlists):
                total_records += 1
                playlist_errors, playlist_warnings = self._validate_playlist(playlist, f"playlist_{i}")
            ...
            for i, setting in enumerate(import_data.settings):
                total_records += 1
                setting_errors, setting_warnings = self._validate_setting(setting, f"setting_{i}")
            ...
            for i, blacklist_entry in enumerate(import_data.blacklist):
                total_records += 1
                blacklist_errors, blacklist_warnings = self._validate_blacklist_entry(blacklist_entry, f"blacklist_{i}")
            ...
            cross_ref_errors, cross_ref_warnings = self._validate_cross_references(import_data)
            ...
            if validation_level == ValidationLevel.STRICT and errors:
                is_valid = False
            elif validation_level == ValidationLevel.MODERATE and len(errors) > (total_records * 0.1):  # More than 10% errors
                is_valid = False
    ...
    def _validate_artist(self, artist: ExportedArtist, record_id: str) -> Tuple[List[ValidationError], List[ValidationError]]:
    ...
    def _validate_video(self, video: ExportedVideo, record_id: str) -> Tuple[List[ValidationError], List[ValidationError]]:
        ...
        # Duration validation
        if video.duration is not None and (video.duration < 0 or video.duration > 86400):  # 24 hours max
            warnings.append(ValidationError(
                record_type="video",
                record_id=record_id,
                field_name="duration",
    ...
    def _validate_playlist(self, playlist: ExportedPlaylist, record_id: str) -> Tuple[List[ValidationError], List[ValidationError]]:
    ...
    def _validate_setting(self, setting: ExportedSetting, record_id: str) -> Tuple[List[ValidationError], List[ValidationError]]:
    ...
    def _validate_blacklist_entry(self, entry: Dict[str, Any], record_id: str) -> Tuple[List[ValidationError], List[ValidationError]]:
    ...
    def _validate_cross_references(self, import_data: ExportData) -> Tuple[List[ValidationError], List[ValidationError]]:
        ...
        try:
            with get_db() as db:
                ...
                if import_data.settings:
                    settings_results = self._import_settings(db, import_data.settings, import_options, progress, update_progress)
                    import_results.update(settings_results)
                
                # Import artists
                if import_data.artists:
                    artist_results = self._import_artists(db, import_data.artists, import_options, progress, update_progress)
                    import_results.update(artist_results)
                
                # Import videos
                if import_data.videos:
                    video_results = self._import_videos(db, import_data.videos, import_options, progress, update_progress)
                    import_results.update(video_results)
                
                # Import playlists
                if import_data.playlists:
                    playlist_results = self._import_playlists(db, import_data.playlists, import_options, progress, update_progress)
                    import_results.update(playlist_results)
                
                # Import blacklist
                if import_data.blacklist:
                    blacklist_results = self._import_blacklist(db, import_data.blacklist, import_options, progress, update_progress)
        ...
        for artist_data in artists:
            try:
                ...
                if import_options.preserve_ids and artist_data.id:
                    ...
                    existing_artist = db.query(Artist).filter(Artist.name == artist_data.name).first()
                    if not existing_artist and artist_data.imvdb_id:
                        existing_artist = db.query(Artist).filter(Artist.imvdb_id == artist_data.imvdb_id).first()
                
                if existing_artist:
                    if import_options.mode == ImportMode.MERGE_SKIP:
                        results["artists_skipped"] += 1
                        continue
                    elif import_options.mode in [ImportMode.MERGE_UPDATE, ImportMode.REPLACE_ALL]:
                        # Update existing artist
                        self._update_artist_from_data(existing_artist, artist_data, import_options)
                        results["artists_updated"] += 1
                else:
                    # Create new artist
                    new_artist = self._create_artist_from_data(artist_data, import_options)
                    db.add(new_artist)
                ...
                progress.overall_progress = min(90.0, 25.0 + (progress.records_processed / progress.total_records) * 60.0)
                update_progress(progress)
                ...
                if progress.errors_count >= import_options.max_errors:
                    raise ValueError(f"Too many errors ({progress.errors_count}), aborting import")
            ...
            try:
                ...
                if import_options.preserve_ids and video_data.id:
                    ...
                    if not existing_video and video_data.youtube_id:
                        existing_video = db.query(Video).filter(Video.youtube_id == video_data.youtube_id).first()
                    if not existing_video and video_data.imvdb_id:
                        existing_video = db.query(Video).filter(Video.imvdb_id == video_data.imvdb_id).first()
                ...
                if existing_video:
                    if import_options.mode == ImportMode.MERGE_SKIP:
                        ...
                        self._update_video_from_data(existing_video, video_data, import_options)
                        results["videos_updated"] += 1
                else:
                    # Verify artist exists
                    artist = db.query(Artist).filter(Artist.id == video_data.artist_id).first()
                    if not artist and import_options.create_missing_artists:
                        ...
                        logger.warning(f"Skipping video {video_data.title}: artist {video_data.artist_id} not found")
                        results["videos_skipped"] += 1
                        continue
    ...
    def _create_artist_from_data(self, artist_data: ExportedArtist, import_options: ImportOptions) -> Artist:
        ...
        artist = Artist(
            ...
            thumbnail_url=artist_data.thumbnail_url if import_options.sanitize_file_paths else None,
            auto_download=artist_data.auto_download,
            monitored=artist_data.monitored,
            folder_path=self._sanitize_path(artist_data.folder_path) if import_options.sanitize_file_paths and artist_data.folder_path else artist_data.folder_path,
    ...
    def _create_video_from_data(self, video_data: ExportedVideo, import_options: ImportOptions) -> Video:
        ...
        video = Video(
            ...
            release_date=datetime.fromisoformat(video_data.release_date) if video_data.release_date else None,
            description=video_data.description,
            view_count=video_data.view_count,
            like_count=video_data.like_count,
            status=VideoStatus(video_data.status) if video_data.status in self.valid_video_statuses else VideoStatus.WANTED,
            quality=video_data.quality,
            video_metadata=video_metadata,
            imvdb_metadata=video_data.imvdb_metadata,
            local_path=self._sanitize_path(video_data.local_path) if import_options.sanitize_file_paths and video_data.local_path else video_data.local_path
        )
    ...
    def _serialize_import_options(self, import_options: ImportOptions) -> Dict[str, Any]:
    ...
    def _import_playlists(
        ...
        results = {"playlists_imported": 0, "playlists_updated": 0, "playlists_skipped": 0}
        ...
        for playlist_data in playlists:
            try:
                ...
                if import_options.preserve_ids and playlist_data.id:
                    existing_playlist = db.query(Playlist).filter(Playlist.id == playlist_data.id).first()
                else:
                ...
                if existing_playlist:
                    if import_options.mode == ImportMode.MERGE_SKIP:
                        ...
                        self._update_playlist_from_data(existing_playlist, playlist_data, import_options, db)
                        results["playlists_updated"] += 1
                else:
                    # Create new playlist
                    new_playlist = self._create_playlist_from_data(playlist_data, import_options, db)
                    if new_playlist:
                        db.add(new_playlist)
    ...
    def _import_blacklist(
        ...
        for blacklist_data in blacklist:
            try:
                ...
                existing_entry = db.query(VideoBlacklist).filter(VideoBlacklist.youtube_url == youtube_url).first()
                ...
                if not existing_entry:
                    ...
                    new_entry = VideoBlacklist(
                        ...
                        created_at=datetime.fromisoformat(blacklist_data.get('created_at')) if blacklist_data.get('created_at') else datetime.utcnow()
                    )
                    db.add(new_entry)
                ...
                # Update progress
                progress.records_processed += 1
                progress.status_message = f"Importing blacklist entry: {youtube_url[:50]}..."
                progress.overall_progress = min(90.0, 25.0 + (progress.records_processed / progress.total_records) * 60.0)
                update_progress(progress)
    ...
    def _create_playlist_from_data(self, playlist_data: ExportedPlaylist, import_options: ImportOptions, db: Session) -> Optional[Playlist]:
        """Create a new Playlist object from ExportedPlaylist data"""
        
        # Verify user exists
        user = db.query(User).filter(User.id == playlist_data.user_id).first()
        if not user:
            logger.warning(f"Skipping playlist {playlist_data.name}: user {playlist_data.user_id} not found")
            return None
    ...
    def _update_artist_from_data(self, artist: Artist, artist_data: ExportedArtist, import_options: ImportOptions):
        ...
        if artist_data.folder_path:
            artist.folder_path = self._sanitize_path(artist_data.folder_path) if import_options.sanitize_file_paths else artist_data.folder_path
    ...
    def _update_video_from_data(self, video: Video, video_data: ExportedVideo, import_options: ImportOptions):
        ...
        if video_data.local_path:
            video.local_path = self._sanitize_path(video_data.local_path) if import_options.sanitize_file_paths else video_data.local_path
    ...
    def _update_playlist_from_data(self, playlist: Playlist, playlist_data: ExportedPlaylist, import_options: ImportOptions, db: Session):
        ...
        if playlist_data.entries:
            ...
            db.query(PlaylistEntry).filter(PlaylistEntry.playlist_id == playlist.id).delete()
            ...
            for entry_data in playlist_data.entries:
                ...
                if video_id and db.query(Video).filter(Video.id == video_id).first():
                    new_entry = PlaylistEntry(
                        ...
                        added_at=datetime.fromisoformat(entry_data.get('added_at')) if entry_data.get('added_at') else datetime.utcnow()
                    )
                    db.add(new_entry)
    ...
    def _parse_artists_csv(self, csv_content: str) -> List[ExportedArtist]:
        ...
        for row in csv_reader:
            artist = ExportedArtist(
                ...
                imvdb_metadata=json.loads(row.get('imvdb_metadata', '{}')) if row.get('imvdb_metadata') else None,
    ...
    def _parse_videos_csv(self, csv_content: str) -> List[ExportedVideo]:
        ...
        for row in csv_reader:
            video = ExportedVideo(
                ...
                view_count=int(row.get('view_count', 0)) if row.get('view_count') else None,
                like_count=int(row.get('like_count', 0)) if row.get('like_count') else None,
                genres=row.get('genres', '').split(', ') if row.get('genres') else None,
                directors=row.get('directors', '').split(', ') if row.get('directors') else None,
                producers=row.get('producers', '').split(', ') if row.get('producers') else None,
                status=row.get('status', 'WANTED'),
                quality=row.get('quality'),
                video_metadata=json.loads(row.get('video_metadata', '{}')) if row.get('video_metadata') else None,
                imvdb_metadata=json.loads(row.get('imvdb_metadata', '{}')) if row.get('imvdb_metadata') else None,
                created_at=row.get('created_at'),
                updated_at=row.get('updated_at'),
    ...
    def _parse_playlists_csv(self, csv_content: str) -> List[ExportedPlaylist]:
        ...
        for row in csv_reader:
            playlist = ExportedPlaylist(
                ...
                total_duration=int(row.get('total_duration', 0)) if row.get('total_duration') else None,
                video_count=int(row.get('video_count', 0)),
                playlist_metadata=json.loads(row.get('playlist_metadata', '{}')) if row.get('playlist_metadata') else None,
                thumbnail_url=row.get('thumbnail_url'),
                created_at=row.get('created_at'),
                updated_at=row.get('updated_at'),
                entries=json.loads(row.get('entries', '[]')) if row.get('entries') else []
            )
    ...
    def _create_backup(self, operation_id: int) -> str:
        ...
        try:
            ...
            from src.database.import_export_models import ExportOptions, ExportFormat, ExportType
            ...
            logger.info(f"Created backup {backup_filename} for import operation {operation_id}")
