...
class AuthentikProvider(OAuthProvider):
    ...
    def map_groups_to_role(self, groups: list, roles: list = None) -> UserRole:
        ...
        for membership in all_memberships:
            ...
            elif any(manager_group in membership_lower for manager_group in manager_groups):
...
class GitHubProvider(OAuthProvider):
    ...
    def get_user_info(self, access_token: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'verified': any(email.get('verified') for email in emails if email.get('primary'))
        }
...
class OAuthService:
    ...
    def initiate_oauth_flow(self, provider_name: str) -> Tuple[bool, str, Optional[str]]:
        """
    ...
    def handle_oauth_callback(self, provider_name: str, code: str, state: str) -> Tuple[bool, str, Optional[User], Optional[UserSession]]:
        """
        ...
        try:
            ...
            if not session_state or session_state != state:
                return False, "Invalid state parameter - possible CSRF attack", None, None
            ...
            provider = self.get_provider(provider_name)
            if not provider:
                return False, f"OAuth provider '{provider_name}' not configured", None, None
            ...
            # Find or create user
            user, session_obj = self._find_or_create_oauth_user(provider_name, user_info)
    ...
    def _find_or_create_oauth_user(self, provider_name: str, user_info: Dict[str, Any]) -> Tuple[Optional[User], Optional[UserSession]]:
        """
        ...
        try:
            with get_db() as session:
                ...
                if not user:
                    ...
                    if not username:
                        username = email.split('@')[0] if email else f"oauth_user_{oauth_id}"
                    ...
                    if provider_name == 'authentik' and isinstance(self.providers[provider_name], AuthentikProvider):
                        groups = user_info.get('groups', [])
                        roles = user_info.get('roles', [])
                        user_role = self.providers[provider_name].map_groups_to_role(groups, roles)
                    
                    # Create user
                    user = User(
                        username=username,
                        email=email or f"{username}@oauth.local",
                        password=secrets.token_urlsafe(32),  # Random password (OAuth users don't use it)
                        role=user_role
                    )
                    ...
                    logger.info(f"Created new OAuth user: {username} from {provider_name}")
                    ...
                    if provider_name == 'authentik' and isinstance(self.providers[provider_name], AuthentikProvider):
                        groups = user_info.get('groups', [])
                        roles = user_info.get('roles', [])
                        new_role = self.providers[provider_name].map_groups_to_role(groups, roles)
                        if new_role != user.role:
                            logger.info(f"Updated user role for {user.username}: {user.role.value} -> {new_role.value}")
                            user.role = new_role
