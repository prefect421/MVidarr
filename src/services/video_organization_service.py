...
class VideoOrganizationService:
    ...
    def process_video_file(self, file_path: Path) -> Dict:
        ...
        try:
            ...
            result['success'] = True
            logger.info(f"Successfully organized: {file_path.name} -> {destination_path}")
            ...
            with get_db() as session:
                ...
                if artist:
                    ...
                    if video:
                        ...
                        logger.info(f"Updated database record for: {artist_name} - {title}")
                    else:
                        logger.debug(f"No video record found for: {artist_name} - {title}")
    ...
    def _auto_index_video_if_enabled(self, artist_name: str, title: str, file_path: str):
        ...
        try:
            ...
            if auto_index:
                ...
                if result['success']:
                    ...
                    logger.warning(f"Failed to auto-index: {artist_name} - {title}: {result.get('error')}")
    ...
    def reorganize_existing_videos(self) -> Dict:
        ...
        logger.info(f"Reorganization complete: {successful} successful, {failed} failed, {skipped} skipped")
        return summary
    ...
    def reorganize_single_existing_video(self, file_path: Path) -> Dict:
        ...
        try:
            ...
            result['success'] = True
            logger.info(f"Successfully reorganized: {file_path.name} -> {destination_path}")
