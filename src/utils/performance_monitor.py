...
def get_performance_report() -> Dict[str, Any]:
    ...
    report = {
        ...
        "all_endpoints": sorted(all_stats, key=lambda x: x["avg_response_time"], reverse=True),
        "generated_at": datetime.now().isoformat()
    }
...
def log_performance_summary():
    ...
    logger.info(f"⚡ Fastest: {fastest['endpoint']} ({fastest['avg_response_time']:.3f}s)")
    logger.info(f"🐌 Slowest: {slowest['endpoint']} ({slowest['avg_response_time']:.3f}s)")